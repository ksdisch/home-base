...
- `runs_summary` computed `thin` over **today**, whose `cost_usd` is a partial sum — `sweeps/envelope.py` appends one row per topic **as each run finishes**. Replayed minute-by-minute against the live 69-row ledger, the false "thin or half-failed" chip fires at **06:04 CT** on this morning's real sweep and clears only when the second topic lands, on the one strip whose entire job is honest trust reporting.
