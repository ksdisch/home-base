...
kanban
  ...
  next["📋 Planned"]
    trust["Wave 1 — trust +<br/>liveness: didnt-run<br/>banner QU12 →<br/>sweep-trust gauge PR5<br/>→ heartbeat outside<br/>the app PR12 (local,<br/>rides last)"]
    qnotes["Wave 1 — notes funnel:<br/>Note button on News +<br/>For You cards QU5 ·<br/>serves the notes/week<br/>v1 criterion"]
...
1. **Wave 1 — trust + liveness + notes funnel** (= the Planned column; the standing picks,
   order confirmed): QU12 didn't-run banner → PR5 sweep-trust gauge → QU5 notes-on-News →
   PR12 heartbeat (**local-only session** — launchd install).
