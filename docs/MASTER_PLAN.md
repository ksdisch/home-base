...
- **RED re-proved by base replay: 22 of the 27 touched tests fail at `3f35a80`.** The other five **pass at base** — each guards a regression a draft introduced, so they are non-regression pins, not REDs; one of the twenty-two is a pre-existing test **tightened** here. Twelve mutations run **one at a time** against the final 72-test set: blanking → 12 dead · negation set → 11 · withholding → 11 · scan order → 10 · named-window-as-time → 5 · whole-note drop → 5 · note-level guard → 2 · day skip → 2 · unconditional day skip → 2 · clause scoping → 1 · gap guard → 1 · repair → 1. The clause-scoping mutation killed **zero** on the first sweep and exposed an unpinned shape; a pin was added and it now kills 1. Backend 929→955 · ruff green.
- **The review found thirteen should-fix defects in the fix**, eleven of them introduced by the drafts, over 7 rounds — a day-exclusion's negation hijacking the next time trigger · an allow-list too narrow for ordinary wording · a gap that swallowed a range opener · that guard then re-inverting ordinary prepositional phrases · and a negated range applied verbatim. **The structural fix that ended the cycle:** a `nothing|never|none` no negated scan could read now withholds the window entirely, because a gap miss produced the *backwards* answer, not no answer — and the guard itself needed two more rounds, to check per occurrence (one readable clause was vouching for an unreadable one) and only within its own clause (a "nothing on Sundays" day phrase was vetoing the window stated beside it). Two nice-to-haves carried as `#192-F1`/`#192-F2`.
