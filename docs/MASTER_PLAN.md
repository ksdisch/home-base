...
weeks are pre-attribution build-blitz traffic. Adversarial review (F1) then showed the graded
week offered only **5 readable mornings**: 07-31 and 08-01 rendered no brief at all (all-8-topic
API errors; 18/21 mornings in the full window rendered — the original "21/21" counted
day-directories). Extended: two attributed weeks (08-03 → 08-16), verdict at the ~08-19
re-grade, certifying on **`mornings_phone` ≥5 measured against readable mornings** (supply
record checked first; <5 readable = supply failure, extends the window) + ≥3 notes/week (a
...
- Adversarial review round 1 corrected the record before merge: **supply side was NOT clean**
  ...
  API errors), two inside the graded week, which offered only 5 readable mornings (F1). The
  ~08-19 verdict therefore grades against **readable** mornings, supply checked first. The
