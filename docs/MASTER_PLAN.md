...
- The osascript test assertion is inverted (`[-, handle, caption]`, no file argv), so a
  ...
  `delivery.json` same-day with a `--force` live verify at rollout (F3). Value stays untracked
  — the config is gitignored PII.
