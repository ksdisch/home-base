...
- **RED re-proved by base replay: 18 of the 23 touched tests fail at `3f35a80`.** The other five **pass at base** — each guards a regression a draft introduced, so they are non-regression pins, not REDs; one of the eighteen is a pre-existing test **tightened** here. Ten mutations run **one at a time** against the final 68-test set: drop the blanking → 11 dead · narrow the negation set → 10 · flip the scan order → 9 · drop the withholding → 5 · make it note-level → 2 · drop clause scoping → 2 · un-count named windows → 2 · drop the day-phrase skip → 2 · drop the gap guard → 1 · revert the repair → 1. Backend 929→951 · ruff green.
- **The review found eleven should-fix defects in the fix**, nine of them introduced by the drafts, over 6 rounds — a day-exclusion's negation hijacking the next time trigger · an allow-list too narrow for ordinary wording · a gap that swallowed a range opener · that guard then re-inverting ordinary prepositional phrases · and a negated range applied verbatim. **The structural fix that ended the cycle:** a `nothing|never|none` no negated scan could read now withholds the window entirely, because a gap miss produced the *backwards* answer, not no answer — and the guard itself needed two more rounds, to check per occurrence (one readable clause was vouching for an unreadable one) and only within its own clause (a "nothing on Sundays" day phrase was vetoing the window stated beside it). Two nice-to-haves carried as `#192-F1`/`#192-F2`.
