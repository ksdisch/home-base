...
- **RED re-proved by base replay: 12 of 13 fail at `3f35a80`.** The thirteenth **passes at base** — it guards a regression the first draft introduced, so it is the non-regression pin, not a RED; one of the twelve is a pre-existing test **tightened** here. Four mutations run **one at a time**: narrow the negation set → 4 dead · drop the gap → 2 · drop the blanking → 5 · flip the scan order → 5. Backend 929→941 · ruff green.
- **The review found four should-fix defects in the fix**, three of them regressions the drafts introduced: a day-exclusion's negation hijacking the next time trigger · an allow-list too narrow for ordinary wording, which the new repair then faithfully honored into a plan *inside* the excluded band · a gap that swallowed a range opener. All fixed and re-verified over 3 rounds. Two nice-to-haves carried as `#192-F1`/`#192-F2`.
