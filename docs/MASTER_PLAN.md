...
kanban
    ...
    bugp1["`Bug fixes P1 — all 5 medium 07-19 hunt findings fixed 07-19, one PR each, test-first: blank-brief PR 73 · promptless scout add PR 74 · frozen news category PR 75 · UTC streak days PR 76 · dev-vs-prod port clash PR 77`"]
  doing["🔄 In progress"]
  ...
  next["📋 Planned"]
    trust["`Trust + liveness cluster — ran-or-didnt banner QU12 · sweep-trust gauge PR5 · heartbeat outside the app PR12`"]
    qnotes["`Notes on News cards QU5 — cross-mode capture, serves the notes/week v1 criterion`"]
  decide["⏸️ Awaiting decision"]
  later["🧊 Later / parked"]
    defer["`Kickoff-deferred: ESPN · auto-courses · alerts · public writing`"]
    moonshots["`4 Moonshot bets — Overnight Chief of Staff · The Mirror · Readiness Brief · Calibrated Doubt wildcard`"]
    quickwins["`3 QuickWins parked — yesterdays brief QU1 · audio resume QU3 · topic chips QU4 — QU5 + QU12 already planned`"]
    premortems["`1 Premortem antibody parked — vault-ritual feed PR10 — heartbeat PR12 + trust gauge PR5 ride the Planned trust cluster`"]
    hardens["`4 Harden guards — re-sweep note detach HA2 · untrusted framing HA4 · feed-drift guard HA8 · migration snapshot HA11`"]
    frictions["`5 Friction smoothings — nav survival FR15 · phone sweep trigger FR2 · audio chapters FR4 · developing context FR13 · undo beat FR10`"]
    buglows["`18 low bug stubs — ranked with fixes in the 2026-07-19 bug-hunt report · BACKLOG Open section`"]
```
