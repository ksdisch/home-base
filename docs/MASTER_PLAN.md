...
kanban
  ...
  doing["🔄 In progress"]
    ...
    wave2["Wave 2 — correctness<br/>sweep: 14 low bugs +<br/>hardens + FR10 · batch<br/>1 pipeline done PR 87<br/>· remaining batches:<br/>LLM-lane · news ·<br/>offline · store ·<br/>notes UX"]
  next["📋 Planned"]
...
2. **Wave 2 — correctness sweep**: the 14 non-courses bug lows + the 4 hardens + FR10 (the 4
   ...
   **Batch 1 (sweep/brief pipeline) ✅ shipped 2026-07-19, PR #87**: #7 unreadable-file
   500 · #22 atomic render staging (renderer stays frozen) · #19 RunAtLoad (live via
   install-schedule.sh from the main checkout post-merge) · #6 query-string dedup
   identity · HA2 re-sweep note guard. Remaining: LLM-lane · news · offline · store ·
   notes UX.
