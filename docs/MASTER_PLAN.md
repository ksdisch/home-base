...
- Fixed as the acceptance names it: a negated bound sets the **opposite** edge (two new triggers, a real negation set + a **≤2-word gap** — Python's `re` forces lookbehinds fixed-width and the gap is what "no sessions after 3pm" needs), and each accepted match is **blanked out** of a working copy before the next scan, the trick `_parse_days` already uses. Blanking preserves length, so **leftmost-per-edge still wins** — the same rule the two single searches had.
- `by` was deliberately left out of the negated-start set ("not by 5pm" isn't a start phrasing); END's bare `by` is protected structurally by the blanking. Two unnamed cases fall out of the same pass: "not until 3pm" (was `{15,15}`) and "nothing before 2pm" (a full inversion).
- **RED re-proved by base replay: 5 fail at `3f35a80`** — 4 new (3 parse + 1 router), plus one **pre-existing test tightened** here, labelled an updated pin rather than a fifth RED. Four mutations run **one at a time**: narrow the negation set → 4 dead · drop the gap → 2 · drop the blanking → 5 · flip the scan order → 5. Backend 929→933 · ruff green.
