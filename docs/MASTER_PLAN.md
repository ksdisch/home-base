...
- **RED re-proved by base replay: 14 of the 15 touched tests fail at `3f35a80`.** The fifteenth **passes at base** — it guards a regression a draft introduced, so it is the non-regression pin, not a RED; one of the fourteen is a pre-existing test **tightened** here. Six mutations run **one at a time** against the final 60-test set: drop the blanking → 10 dead · narrow the negation set → 8 · flip the scan order → 8 · drop the withholding → 3 · drop the gap guard → 3 · revert the repair → 1. Backend 929→943 · ruff green.
- **The review found seven should-fix defects in the fix**, five of them regressions the drafts introduced, over 4 rounds — a day-exclusion's negation hijacking the next time trigger · an allow-list too narrow for ordinary wording · a gap that swallowed a range opener · that guard then re-inverting ordinary prepositional phrases · and a negated range applied verbatim. **The structural fix that ended the cycle:** a `nothing|never|none` no negated scan could read now withholds the window entirely, because a gap miss produced the *backwards* answer, not no answer. Two nice-to-haves carried as `#192-F1`/`#192-F2`.
