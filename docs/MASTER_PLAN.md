...
- **RED re-proved by base replay: 24 of the 29 touched tests fail at `3f35a80`.** The other five **pass at base** — each guards a regression a draft introduced, so they are non-regression pins, not REDs; one of the twenty-four is a pre-existing test **tightened** here. Fourteen mutations run **one at a time** against the final 74-test set: withholding → 13 dead · blanking → 12 · negation set → 10 · scan order → 10 · whole-note drop → 7 · named-window-as-time → 6 · unconditional day skip → 4 · its trigger half → 4 · skip removed → 3 · note-level guard → 2 · its named-window half → 2 · clause scoping → 1 · gap guard → 1 · repair → 1. The clause-scoping mutation killed **zero** on an earlier sweep and exposed an unpinned shape; a pin was added and it now kills 1. Backend 929→957 · ruff green.
- **The review found fourteen should-fix defects in the fix**, twelve of them introduced by the drafts, over 8 rounds — a day-exclusion's negation hijacking the next time trigger · an allow-list too narrow for ordinary wording · a gap that swallowed a range opener · that guard then re-inverting ordinary prepositional phrases · and a negated range applied verbatim. **The structural fix that ended the cycle:** a `nothing|never|none` no negated scan could read now withholds the window entirely, because a gap miss produced the *backwards* answer, not no answer — and the guard itself needed two more rounds, to check per occurrence (one readable clause was vouching for an unreadable one) and only within its own clause (a "nothing on Sundays" day phrase was vetoing the window stated beside it). Two nice-to-haves carried as `#192-F1`/`#192-F2`.
