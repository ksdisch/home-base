...
**Last updated:** 2026-08-13 · **08-12 hunt bug #5 fixed (PR #192, test-first) — negated time preferences no longer parse backwards, after a nine-round review that found sixteen should-fix defects, fourteen of them introduced by the fix's own drafts — one of which was ultimately **reverted** rather than fixed a fifth time.** One defect in `_parse_window`, two confirmed findings. (a) The start trigger guarded `after` with two **literal lookbehinds** — `(?<!not )(?<!no )` — which can only see the word immediately before it, so "nothing after 3pm" / "never after 3pm" / "no sessions after 3pm" all became `day_start_hour: 15`, scheduling **precisely the band the note ruled out**; and because `set_study_prefs` runs before the plan, the inverted window was persisted first and every later visit hydrated to it. (b) The end trigger carries a bare `before`/`until` and its search ran over the **untouched** text, so it fired *inside* the start phrase already consumed — "not before 2pm" → `{14, 14}`, "evenings but not before 7pm" → `{19, 19}`, destroying the named window's end. A negated bound now sets the **opposite** edge via two new triggers (Python's `re` requires lookbehinds fixed-width, which is why the originals were literals), and every scan runs against a working copy with **every match blanked out** — `_parse_days`' own trick — **repeating until its pattern is exhausted**, so no occurrence survives into a later opposite-direction scan; blanking preserves length, so the **leftmost match per edge still wins**, as the two single searches did. **The load-bearing detail is that there are two negator classes**, split on whether the same word can also be spent elsewhere in the note: `no`/`not` are `_EXCLUDE_RE` day-exclusion triggers *and* carry an availability reading ("I have no meetings after 3pm" means those hours are free), so they only flip a bound across an **allow-list** of scheduling nouns; `nothing`/`never`/`none` are neither, so they read any ≤2 words — minus any word that opens a bound of its own or starts with a digit, which would otherwise swallow a range's opener into a zero-width window. One router change rides along: `_apply_overrides` repairs an inverted window by moving the edge the note did **not** name, so an end-only note can't be repaired back into the band it excluded. Two cases the card didn't name fall out too: "not until 3pm" (was `{15,15}`) and "nothing before 2pm" (a full inversion). **RED re-proved by base replay: 24 of the 27 touched tests fail at `3f35a80`**; the other three **pass at base** and are labelled the non-regression pins they are (each guards a regression a draft introduced), and one of the twenty-four is a pre-existing test **tightened** here. Ten mutations applied **one at a time** against the final 72-test set (withholding 13 · blanking 12 · scan order 10 · negation set 10 · whole-note drop 7 · named-window-as-time 5 · note-level guard 3 · clause scoping 3 · gap guard 1 · repair 1) — and one of them earned its keep twice: dropping the clause scoping killed **zero** on the first sweep, exposing an unpinned shape rather than a redundant guard. **The review is the story here:** nine rounds, sixteen should-fix findings, fourteen of them introduced by the drafts. The structural lesson is the keeper — each retune of the negation gap traded one inverted phrasing set for another, because a gap *miss* produced the **backwards** answer rather than no answer, and a non-empty parse short-circuits the `claude -p` lane. So an **unread** `nothing|never|none` now drops the **whole note**, not just the window — per occurrence, scoped to its clause, counting a named window as a time. That last part is load-bearing: withholding only the window was never the honest floor it claimed to be, because the days and session length kept the note non-empty and so short-circuited the very fallback lane the withholding existed to reach. **Four further rounds went to one exception** — a day exclusion shouldn't veto a window beside it — tried as a punctuation test, a separator list, a clause-content test and a trigger word-list, each closing one shape while re-opening another. It was **reverted, not fixed a fifth time**, on a stopping rule agreed in advance: the distinction is semantic and every lexical proxy collides with a sentence meaning the opposite. Carried as `#192-F3` with the evidence for all four attempts. That also settles the negated range ("nothing from 9am until 5pm" is a workday — applying it would schedule every block inside the excluded band). Backend 929→955 · ruff green · frontend untouched. Prior: **PR #190's three review follow-ups carded — plus PR #185's one still-open finding, carded late (docs-only).** #190's loop came back CLEAR (0 critical / 0 should-fix / 3 nice-to-have, nothing disputed, no judge), and its three findings are now backlog items rather than PR-comment-only: **#190-F1** (`BriefMirror` still ships `mornings_phone` with no `attributed` beside it, so the *next* consumer of that payload reintroduces the very fabrication #190 deleted) · **#190-F2** (the "pre-v14 SW-cached payload" rationale is false — `sw.js` matches `["/api/brief","/api/brief/audio"]` by **exact pathname**, so `/api/brief/habit` is never cached; **pre-existing**, not introduced by #190, and no stated decision rests on it) · **#190-F3** (the new `store/db.py` docstring claims a **day-level** guarantee for a **week-level** gate; unreachable from the live write path, so the proportionate fix is softer wording, not a per-day gate). **The late catch:** PR #185's `F1` — a persisted `23/24` window is reachable now that hour 24 plans, but `PathPlayer`'s selects stop at 20/22, so **both dropdowns hydrate blank** — had been sitting in that PR's review comment since 08-12 because #185 merged before the block-per-PR convention existed. Carded as **#185-F1**; that review's other finding was fixed twice inside the PR and closed VERIFIED, so it isn't carried. Prior: **08-12 hunt bug #4 fixed (PR #190) — the Habit strip no longer fabricates "0p" for weeks nothing measured.** `brief_habit_weeks` lacked `mirror.py`'s `attributed` gate, so "no `source` column data" and "measured zero phone reads" were the same integer 0 and July rendered `5m / 0p` — the accusation `mirror.py`'s own comment refuses to make. Fixed with a per-week `attributed: bool` (the sibling reader's shape, and additive — a JSON `null` would have slipped past HabitStrip's `!== undefined` guards). Per-week because both cases are live: 07-13/07-20 are all-NULL false zeros while 08-03 is a **true** zero that must keep rendering, or the fade signal v14 exists to catch gets hidden too. The v1 criterion is untouched and the ~08-19 verdict was never at risk — its weeks (08-03, 08-10) were already attributed. Backend 928→929 · frontend 266→268 · ruff/tsc green; RED re-proved by base replay (5 fail at `af7f230`). Prior: **PR #188's two review follow-ups carded + the fix deployed to the prod hub (docs-only diff)** — the nice-to-have findings from #188's pre-merge loop are now backlog items rather than PR-comment-only: **#188-F1** (`thin` measures dollars as a proxy for completeness, plus no `judged`/`pending` marker on `BriefRunDay`) and **#188-F2** (`missing_days` has the very defect #188 fixed — an amber "no sweep: today" every morning before ~06:04). #188's third finding was upgraded to should-fix and fixed pre-merge, so it isn't carried. **#188-F1 is carded in a materially better form than the review produced it, and the credit is Kyle's:** he challenged the "half-failed morning" reading of `2026-08-09`, suspecting the off-season roster pause. The ledger says the pause is real but lands **between 08-11 and 08-12** (08-11 ran all 9 at $19.48; 08-12/08-13 ran exactly 6 at $9.73/$9.09), so it explains those days and **not** 08-09 — which ran `st-louis-blues` while it was still active, at catch-up hours (22:38Z then 05:43Z, vs 11:03–11:46Z on every healthy day), after 08-07/08-08 left no rows at all. **But the challenge proved "thin" is the wrong word for it:** 08-09's cost *per topic* is **$1.23**, against a $1.51 median and statistically identical to 08-06's **$1.24** on a complete 9-topic day — the three topics that ran ran **normally**, and the pathology is that **six of nine never ran**. Cost is a lossy proxy for completeness in both directions (misses a two-thirds outage; would misread a deliberate roster cut as degradation), and `topics` is already in the payload, so the fix needs no roster coupling — which also dissolves the pause-flag objection raised during that PR's triage. One win already banked: #188's median change means 08-09 **is** flagged at `days=14` ($3.69 < $3.97) where the old code missed it ($3.38); it still escapes at the strip's default `days=7`. **Deployed:** the prod hub was 2 commits behind and its `dist/` dated **Jul 27**, so it had been serving a stale frontend for over two weeks (PR #186's `NotebookCard` fix included) — fast-forwarded to `12329d5`, `npm run build`, `com.homebase.server` kickstarted; health 200, served asset `index-DL4w1-Dn.js` matches the new build, and the live `GET /api/brief/runs/summary` confirms today unflagged. Prior: **2026-08-12 hunt bug #3 fixed — the sweep strip called the in-flight morning "thin or half-failed" (PR #188, test-first)** — `runs_summary` flagged `thin` off a comparison it ran on **today**, whose `cost_usd` is a partial sum: `sweeps/envelope.py` appends one ledger row per topic **as each `claude -p` finishes**, so a $1.86 first topic measured against a ~$10 norm reads as a half-failed sweep during exactly the window Kyle is reading it. Replaying the live ledger (181 rows over 24 days at the time of the replay) minute-by-minute through this morning's real sweep, the false chip fires at **06:04 CT** and clears only when the second topic lands — on the one strip whose entire job is honest trust reporting, and it recurs every morning the read overlaps the sweep. Fixed by the acceptance's first option: **only days strictly older than `end` are judged**, excluded from **both** the median and the flag. The second option — gate on the day being "complete" — was ruled out on evidence: the ledger carries **no completion sentinel**, and the only end-of-sweep filesystem artifacts (`brief.mp3`, `brief.chapters.json`) come from the **best-effort** audio step explicitly allowed to fail without failing the sweep, so gating there would make the trust readout depend on TTS, while a roster-size comparison would couple it to `topics.json` and its pause flags. "Strictly older than `end`" is the only completeness predicate actually knowable at this seam. **Excluding the current day from the median is the second half of the same defect, not a bonus** — an in-flight day is not a sample of what a day costs, so it biases the norm toward zero: demonstrated at $0.50 pulling a $9.00 norm down to $2.00 and silently **un-flagging two genuinely thin past days**, the exact masking the `thin` flag exists to prevent. **The pre-existing `test_a_day_far_under_the_norm_is_flagged_thin` pinned the bug** — it passed `today=` the very day it asserted thin — so it was rewritten to put the tell on a past day; it passes at base too and is labelled the non-regression pin it is, not counted as RED. **RED verified by replaying the final tests against base `c0b5505`: both new tests fail there**, and each was mutation-checked *in isolation* and fails only for the half it pins (median-only mutation → the masking test · flag-only mutation → the in-flight test) — an earlier combined mutation run mis-attributed its output and was re-run rather than trusted. **Accepted cost, stated rather than buried:** a genuinely half-failed sweep today is not chipped `thin` until tomorrow — the `latest` line still reports today's topics, duration, cost and errors directly, so it stays visible, just not with that word. Backend 926→928 · ruff green · frontend untouched (the flag's shape is unchanged). Prior: **PR #186's four review follow-ups carded (docs-only)** — the nice-to-have findings from that PR's pre-merge loop are now backlog items instead of living only in a PR comment: **#186-F6** (the 422 copy prints the server's absolute filesystem path, and its test asserts a string the server never emits — the same class as the F2 that caught the overstated RED count) · **#186-F3** (the `.bak` is a single slot, so a second confirmed replace destroys what the backup exists to save) · **#186-F4** (a failed `.bak` copy loses a paid generation as a 500 the endpoint's "never a 500" docstring denies) · **#186-F5** (the 409 names a param no client can send; recorded alongside the reviewer's point that the 409 is unreachable from the UI by construction, so it is API-only defense-in-depth). Filed in their own `### Review follow-ups` section and numbered **by PR + finding** (`#186-F3`) rather than a bare integer — deliberately, because the three hunts' bare numbers already collide with each other and a fourth series would compound it. Prior: **2026-08-12 hunt bug #2 fixed — ✨ Generate could silently overwrite the hand-authored path (PR #186, test-first)** — the only finding whose worst case was **irrecoverable**: `generate_path` called `write_path_file` with no existence check, and that function is tempfile + `os.replace` with no backup — so one tap could destroy the real **37-step hand-authored** sidecar in the gitignored `backend/data/paths/`, which the Designer provably cannot reproduce (it emits neither `video` nor `mindmap` and caps at 8 audio / 3 quiz, so any regeneration is strictly lossy). The trigger needed no user error: `api.path()` resolves null **only** for a 404 and rethrows everything else — precisely so a caller can tell "absent" from "broken" — but `NotebookCard` collapsed both with `.catch(() => setPath(null))`, so any transient non-404 rendered the NoPathState whose sole control is an **unconfirmed** Generate. **The sharpest case is inverted:** a *malformed* existing file is the state most likely to show that button (the GET 422s), and it is exactly the file only a human can repair. Guarded at **both** layers rather than one: `generate_path` gains `replace: bool = False` and **409s** when `path_file(notebook_id)` is non-None — keyed on the examples ∪ user union (shadowing a bundled example is deliberate too), placed **before** `compose_path` so refusing costs neither a minute nor a cent — and `write_path_file` now `shutil.copy2`s any existing destination to `<id>.json.bak` **inside its try**, so failing to preserve the old file aborts the write instead of destroying it (`.bak` stays out of `_path_files`, which globs `*.json`). The card carries the error instead of discarding it: a **422** (the file is on disk and unparseable — permanent and hand-fixable) names itself with the parser's own detail, and only a genuinely transient failure gets transient wording (review F1 — the first pass said "just now" about a permanent state, the same class of surface PR #182 fixed). Verified **read-only** that `path_file` discovers the real 37-step sidecar, so the guard covers it — the endpoint was never pointed at real data. Nothing sends `replace=true`: the confirmed-replace lane is API-only by design, since wiring a confirm affordance into the button exceeds the finding's acceptance — and review F5 sharpened why that's sound, since the 409 is **unreachable from the UI** by construction (NoPathState renders only on a 404, exactly when the guard cannot fire), making it API-only defense-in-depth with all UI-side protection coming from the card branch. Distinct from known-open 07-26 #6 (stale `path_step_progress` rows on regen), deliberately not folded in. **RED verified by replaying the final tests against the base commit: 8 of 9 fail there** (6 backend · 2 card); the ninth is a non-regression pin (`first_write_leaves_no_backup`) and is labelled as one — the first pass claimed "all 8", which review F2 disproved by the same replay, and that claim was wrong in the same way PR #185's was. The backup-discovery pins were also mutation-checked: widening `_path_files`' glob to `*` fails both, which the original endpoint assertion could not have caught. Backend 919→926 · frontend 264→266 · ruff/tsc green. Prior: **2026-08-12 hunt bug #1 fixed — the studycal hour-24 propose-500 (PR #185, test-first)** — `_local_window` was `day.replace(hour=hour, …)`, whose domain is 0..23, while `day_end_hour` is an **exclusive** end that every caller clamps to **1..24** (`api/study.py`'s controls and `_apply_overrides`, `negotiate.py`'s knobs) and that the inverted-window repair `min(24, day_start_hour + 1)` **manufactures outright** — so a 23:00 start, or an explicit `day_end_hour: 24`, raised `ValueError: hour must be in 0..23` as a hard 500. Worse, `set_study_prefs` runs **before** `plan_sessions`, so the unrepresentable window was persisted first and every later propose — even an empty body — 500'd on the stored prefs, out of reach of the panel's 6–20 / 7–22 selects. Fixed at the receiving end, not by narrowing the clamps: `_local_window` now offsets from local midnight (`replace(hour=0…) + timedelta(hours=hour)`), the same form the sibling `_events_in_window` already used, so hour 24 means "closes at local midnight". Clamping to 23 was rejected — it would silently eat the last hour of a legitimate "study until midnight" window and leave a 23:00 start unrepairable. **Behaviour for 0..23 is unchanged on every reachable path** — instant-identical across both 2026 DST transitions for `fold=0` anchors. Stated precisely because the first pass overstated it (caught at review as F2): the equivalence is *not* unconditional — `aware + timedelta` resets `fold` where `replace` preserves it, so ambiguous-hour anchors land 3600s apart — but it is inert in `plan_sessions`, where every `d >= 1` is already `fold`-reset by `+ timedelta(days=d)` and the sole `d == 0` divergence is dominated by `eff_start = max(win_start, now, …)`. No separate migration for already-persisted 23/24 prefs: once 24 plans, those are a legitimate 11pm–midnight window rather than poison — pinned by a router test that re-proposes with an **empty body** against the stored prefs and asserts the same `applied`. RED shown on all three tests before the one-line fix. Backend 916→919 · ruff green. Prior: **Backlog replenished via `/replenish` (docs-only)** — the dry pipeline (2 open ideas · 5 open bugs, all solidification) refilled from a four-lane run (bug-hunt over the ~40-commit post-07-26 delta + Moonshot(tethered)/QuickWin/Harden, 69 agents, every finding adversarially verified, every idea through a two-sided critic gate + refine): **22 verified bugs** ([report](bug-hunt/2026-08-12-post-0726-delta.md) — 1 high: studycal hour-24 propose-500 with the poisoned window persisted first · 9 medium incl. Generate silently overwriting the hand-authored 37-step path sidecar, the runs strip calling the in-flight morning "thin", and the habit strip's fabricated "0p" on pre-v14 weeks · 12 low) and **13 refined ideas** captured (vision doc + stub each): 4 Moonshot [Exploration]/L under the lane freeze (Talk Back — 4-lens convergence · The Docket · Retraction Ledger · Belief Ledger wildcard) · 5 QuickWin incl. the 3-lens delivery receipt and three ~08-19-verdict metric-integrity fixes · 4 Harden incl. the outbound send gate and the untrusted-fence invariant (**which IS open bug #18's fix — the 07-26 hunt's designer-prompt #18, not this hunt's — don't fix that bug separately**). Cross-lane fold: the Harden calendar-reconcile guard became bug #11's suggested fix. Prior: **Arc C shipped — the two shipped surfaces that stated a guarantee they didn't keep (PR #182, test-first)**. **#16 Overnight approve/discard** was check-then-act: resolve → `add_brief_note` → `append_status` are three unguarded steps on FastAPI's threadpool, so two taps could both clear the read and both write a real note — the ledger self-heals on read (`load_queue` honours the first status row only), a duplicate note does not. A module-level `_overnight_lock` (the `_sweep_lock` pattern from the same file) now wraps all three steps on **both** verbs, and `_resolve_overnight`'s docstring stops claiming a promise it can't keep alone. Client half: per-proposal `overnightPending` disables both verbs between tap and response, cleared in `.finally()` so a failure re-arms instead of stranding the draft. **#11 Progress** blanked on the one rejection it couldn't survive — its own: `Promise.allSettled` degraded correctly but both body branches gated on `data`, so a progress-only failure rendered a header over nothing while review, reflections and paths sat loaded and unreachable. The body now gates on `hasAnything`, the four `/progress`-reading blocks null-guard individually (`ThreeAxisBand` is the only one that truly needs it), and the banner tracks progress's own rejection, saying "the other sections below still loaded" when they did. **The RED was real and deterministic** — the concurrency tests fail **5/5** runs at `28a9dbd` ("a double tap wrote 2 real notes") via a `_park_after_resolve` barrier helper that holds every tap clearing the read until a second one does, so the race is provable rather than hopeful; a crossed-verb approve-vs-discard race is pinned too. Backend 914 (this PR +3) · frontend 264 (+5) · ruff/tsc green. Bugs now **42 of 47 fixed, 5 open** (#6 · #15 · #17 · #18 · #19). Prior: **The brief gets delivered (PRs #181 + #183)** — a best-effort post-sweep `sweeps/deliver_brief.py` emails the morning MP3 (Gmail SMTP, app password from the Keychain) and texts a caption + tap-to-play tailnet audio link via this Mac's own Messages.app (text-only by design: macOS silently drops AppleScript file sends, proven on the first live run); self-addressed only, config gitignored as PII, zero LLM in the path. **LIVE** — setup done and the first real email + (pre-fix, link-less) text delivered 2026-08-12; `base_url` set in the live config the same day, and the link path's live verification runs at this PR's rollout (ff + server kickstart + `--force` re-send; result recorded in the PR). Prior: **Backlog re-counted, plus two live findings that outrank it** (`/backlog-hygiene` — [full brief](backlog-hygiene/2026-08-11.md)) — bugs re-verified against the code, not their own text: **40 of 47 fixed, 7 open** (#6 · #11 · #15 · #16 · #17 · #18 · #19 — all S-sized, none test-covered; **#17's shape enlarged by PR #173**, which shipped `video`/`mindmap` frontend-only, so both now render as first-class rateable steps while staying unvalidated server-side). Ideas: **2 open** (`roster-entropy` · `feed-the-vault-ritual`). Doc-truth corrections: episode-review had claimed "In progress" for ~2 months after shipping, and the course epic's "next" list was 4-of-5 shipped — **only live Mermaid rendering remains**. PRs **#145** (superseded by #153; merging it would have been **−9434 lines**, deleting the Wiki, LICENSE, `visit_source.py` and ~50 test files) and **#125** (premise reversed — course content is gitignored by design) **closed, not merged**. **Two findings outrank the backlog.** *Supply:* the 06:00 sweep failed 4 of the last 12 mornings on two separable causes — a **weekly-quota wall recurring as Fri+Sat pairs exactly 7 days apart** (next due 08-14/15, **inside the graded window**) and mid-response connection drops burning ~$1–2/topic, with **no retry anywhere in `sweep.sh`**, which cannot tell "retryable after the 11am reset" from "malformed JSON". And **failed-topic spend is never ledgered**: 08-09 records $3.69 against **$12.38 actually spent**, so PR #163's cost readout under-reports **70% on the worst day** and its `thin` flag marks a near-full-price catastrophe as cheap — while the heartbeat, which measures silence rather than completeness, logged "ok" with 6 of 8 topics dead. *Demand:* week 1 of the graded window (08-03→08-09) recorded **0 phone mornings** (bar ≥5) and **1 note** (bar ≥3); the last note was written 08-03, and there have been 6 visit-days in the 16 since attribution began. Instrumentation verified healthy (98 visits lifetime, 2 phone visits on 08-11). Under D12's amendment the supply wobble would classify this as a *supply* failure and extend the window a third time. **Roster: `celtics`, `st-louis-blues`, `kansas-basketball` paused** — all out of season, ~$7/day of a ~$19/day burn, reversible in one tap. **Arc C picked** (#16 Overnight double-tap can double a real note · #11 Progress blanks when one of four calls fails). The one input only Kyle has — whether he was away 08-03→08-10 — is deliberately left unanswered rather than assumed. Prior: **Research-portfolio course regenerated over all eight projects** — the `research-portfolio` hub course refreshed in place per [`PORTFOLIO_COURSE_REFRESH_PLAN.md`](PORTFOLIO_COURSE_REFRESH_PLAN.md): 6 → **8 projects**, 10 → **16 lessons**, ~5h → **~8h**, `validate` ok with zero warnings and live-verified at `/courses/research-portfolio`. Authored from the repos and all eight research papers rather than the portfolio cards — two of which were stale and were fixed under their own PRs (ksdisch/portfolio#11 · ksdisch/hush-gauge#20). NotebookLM gained hush-gauge + its paper and an **S2 Ep 4** episode. Prior: **v1 success-criteria check — ruled NOT YET, extended to the ~08-19 re-grade (D12, docs-only)** — at the ~08-03 check (3 weeks post-kickoff to the day), the only attribution-clean week — 07-27, also the week Kyle was away from the project — failed every measurable bar (**3 raw / 2 phone** mornings vs ≥5 · **2** notes vs ≥3), while the two raw-passing weeks (5 · 5 mornings) predate #159's source attribution and ride July's build blitz (17 visits on 07-16, 18 on 07-21) — certifying them would stamp build exhaust as a reading habit, exactly the failure [builder-vs-reader-metric](ideas/builder-vs-reader-metric.md) predicted. Kyle's ruling: **NOT YET** — neither PASS nor FAIL. Two attributed weeks run 08-03 → 08-16 and the verdict lands at the **~08-19 re-grade** (the two gates collapse into one); the certifying number is **`mornings_phone` ≥5** (reader-Kyle) — error model recorded both ways: desktop-over-tailnet over-counts as `phone`, while reads of the deployed build on the Mac file as `mac-localhost` and certify as **zero** (07-28's six real opens count for nothing), an accepted under-count with phone-only certifying — and the notes bar stays ≥3/week (a saved chat answer counts once, never as note *and* question). The two testimony criteria (significant events reach Kyle from elsewhere first ≤1/week · foraging → ~zero) went deliberately unanswered — Kyle tallies both live over the window instead of reconstructing them. **The moonshot-lane freeze and the Agent Gate park (D7) hold until the extended verdict.** **Supply side NOT clean** (adversarial review F1 — "21/21 days" had counted day-directories, not briefs): 18/21 mornings rendered a brief; 07-24, 07-31, 08-01 rendered nothing (all-8-topic API errors, zero ledger rows), the latter two inside the graded week — a live supply-side wobble on the project's stated riskiest assumption. Those mornings served the last renderable day's brief under the stale marker (07-30 on both — F11), and opens still counted (visits stamp the visit day — F8): the metric ceiling stayed 7, freshness dropped to 5 of 7. Amendment at review (Kyle's gate intent in F8's two-step wording): readable mornings counted first at the ~08-19 verdict; <5 readable **and** `mornings_phone` <5 = supply failure, extends the window, never a habit failure — `mornings_phone` ≥5 despite outages = bar met, Kyle decides whether to bank it. Suites at the check: backend full pass + frontend 259/259. Decision **D12**. Also open: M6 phone trio (Kyle) · extended v1 verdict ~08-19 · PR10 only-if-wobble. Prior: **Research-portfolio study path + video/mindmap step kinds (PR #173)** — the interview-prep notebook's hub path had been broken since the 2026-08-01 notebook refresh (three steps cited study-aid artifact ids deleted+regenerated that day) and knew only S1 audio — 24 of the notebook's 32 artifacts didn't exist when it was authored. Kyle's call (from three proposed structures): **supersede, don't repair** — a hand-authored 37-step phased path now covers all 32 artifacts (Orient: Study Guide → mind map · S1 lap: 8 eps with a **soft-gated quiz after every episode** — 6/8, honor-system, stated in the step body — and whiteboard videos interleaved by theme · Skeptic Grilling I · S2 lap: mute-map + the 12×12-matrix video, The Engine Revisited · adversarial finale: Debate → Critique → Grilling II · drill: flashcards → synthesis quiz → reflect), "six projects" corrected to seven throughout, stable `ep1`–`ep8` ids preserving any recorded S1 progress. The player learns **`video`/`mindmap` step kinds** (🎬 Watch · 🧠 Mind map, NotebookLM launches, rateable) — D6-honest: the spine stays audio, videos enter as supplementary steps under their **real** kind/artifact_type rather than the #143 mislabel or omission. The path JSON is gitignored user data, so it rides outside the diff: validated via the backend loader + a **type-aware** 32/32 check against the hub's own catalog ingest — every artifact id resolves AND its catalog type matches its step kind, the same class of check the Designer's M0 bar runs on generated paths (adversarial review round 1 caught the one mismatch an id-only diff had missed: the Study Guide was cataloged `report`, so the path's only `read` step 404'd by design — sidecar type cell corrected, study-guide route re-verified live `ok:true`) — and already live (the server reads path files per request). Decision **D11**. Frontend tests 258→259 · tsc green · backend untouched (path suite 62/62). Prior: **Visit-source attribution (PR #159)** — the ~08-03 v1 check reads `brief_visits`, and those rows carried no origin, so every localhost dev load, Playwright run, and verify pass of July's build blitz counted as a "morning visit"; the check was on track to certify build exhaust as a reading habit ([idea](ideas/builder-vs-reader-metric.md)). Landed test-first a week ahead of the gate: `app.visit_source` threads FastAPI's `Request` into the visit write and files each load in one coarse bucket (tailnet v4 **and** v6 → `phone` · loopback → `mac-localhost` · the Vite `:5173` origin → `dev`, ahead of the IP · TestClient → `test` · `lan`/`other`/`unknown`), stored in a nullable `source` column at **schema v14**. `mornings_phone` now rides beside `mornings` in `brief_habit_weeks`, the Habit strip, and the Mirror's sentence — **the v1 criterion is deliberately unchanged; both numbers are reported and which one certifies v1 is Kyle's call at the check.** Two honesty gates: July's rows are not back-classified (nullable = unattributed, not zero), and the Mirror omits the phone clause entirely for a window with no attribution rather than fabricating "0 on your phone". Backend +36 · frontend +4. Deployed same-day so attribution accrues real mornings before 08-03. Prior: **Study Scheduler correctness wave (PR #152)** — the eight studycal findings from the 07-26 hunt fixed test-first, ahead of the ~07-29 Google-token expiry that made #2 live. Root cause behind half the cluster: `FakeCalendarPort` always succeeds and hides written events from free/busy, so a mid-batch write failure and a duplicate re-propose were unreachable in tests — a `FlakyCalendarPort` + `feed_back` mode now make them testable. An expired token degrades honestly instead of 500ing behind `connected: true`; every created event is ledgered before the next is created; opt-in is enforced server-side; the parser stops inverting "not Mondays or Fridays" and "9 to 5pm"; placement is curriculum-ordered; a revisit no longer duplicates blocks (schema v13). Backend 739→779. Prior: 2026-07-22 · **Topic↔course cross-links + course quizzes join the daily Plan (this PR)**. The Learning and Courses tabs read as duplicates (the Jacobian topic + its course cover the same paper) and course-quiz mastery decayed silently — course SM-2 rows (written under `course:<slug>`) were filtered out of the study plan. This PR (a) surfaces the already-authored topic↔course link on **both** cards — a topic card gets a `📘 Course →` chip (from a `courses_by_notebook()` reverse index stamped on the catalog), a course card gets a `🎧 Source notebook →` chip (resolved against the sidecar catalog) — and (b) folds course **quizzes** into the cross-notebook `/study-plan` (relaxed `sr_plan_items`' `course:%` filter; course segments titled from the course catalog, badged "Course", routed to `/courses/:slug/quiz`; course flashcards stay on their own surface via a quiz-only filter). `/review` + `/progress` + the home mastery badge stay topic-only. Backend 743→748 · frontend 185→193 · ruff/tsc green. Prior: **M8 Learning Paths fix — build tracks on the AUDIO overviews, not the video (PR #143)**. The live Jacobian-Lens path was quietly built on the notebook's **whiteboard video** season (4 explainer eps: "Anatomy of the Lens" → "Replicating the Paper") mislabeled as `audio` — a hand-authored fixture whose wrong `artifact_id`s bypassed the Designer's catalog cross-check (that check only runs on *generated* paths). Rebuilt around the real **6-episode audio deep-dive season** ("The Global Workspace Idea" → "Consciousness & Replication") as the learning spine; the video series is now supplementary, surfaced only as an optional pointer in the Reflect step. Encoded the convention as **design decision 12** (audio primary, video supplementary — the Designer already enforces it structurally by only arranging audio/study_guide/flashcards/quiz) + a **fixture guard test** (`test_paths_fixture.py`: audio steps must cite audio-series ids · no step may cite a video-series id · artifact_type must match its kind) so a video-as-audio inversion can't recur. Path is now 11 steps (was 9). Backend 741→743 · ruff green; four fixture-count test expectations updated to match. Prior: **Study Scheduler v1.2 — flag calendar conflicts + double-book (PR #142)**. When a requested study window is already booked, the proposal now **flags the conflicting events by name** (titled `port.busy_events` via the primary calendar's `events.list` — the freebusy API has no titles) and offers **"Book over it anyway"** (`allow_double_book` → `plan_sessions(ignore_busy=True)`), with each block that lands on an event badged *"⚠ double-books X"*. For Kyle's shared-calendar case (his girlfriend adds items he can study through). Placement respects busy by default; double-book is an explicit per-proposal opt-in; `busy_events` is best-effort (freebusy still drives placement). Backend 737→741 · frontend 185→186 · ruff/tsc green; **verified live** — his 2–5pm weekday window flags the real conflicts ("Bright Horizons"/"Gearhead") and double-book yields 2 badged blocks. Prior: **Study Scheduler v1.1 — the note box works (PR #141)**. Live-testing v1 exposed that the free-text box did nothing: the `claude -p` lane it depended on can't reach the CLI from the always-on server (`"claude CLI not found"` in the ledger — same PATH class as the #139 nlm fix), and the UI *lied* by showing the untouched default as the answer. Kyle's call: a **local deterministic parser** (`app.studycal.parse`, no LLM) is now the note box's primary engine — days/exclusions · time-of-day · session length · max blocks, refining the current controls; the note **refines** them (note wins for keys it names) and only a phrasing the parser can't read falls back to `claude -p` (symlinked onto the server PATH); if neither reads it, the plan is left unchanged with an **honest** message. Default window aligned to a daytime 9am–5pm. Backend 723→737 (+parser suite, +refine/fallback/honesty API tests) · frontend 185 green · ruff/tsc green. His exact note ("sixty-minute blocks every weekday, no earlier than 2pm, no later than 5pm") now yields weekdays · 2–5pm · 60-min. Prior: **Study Scheduler v1 — flexible, preference-honoring scheduling (PR #140)**, on top of v0 (PR #137/#138). v0 ignored the two things a learner asks for most — *which days* and *what time of day*: there was no day-of-week concept anywhere, and the evening-only window silently rewrote "before 2pm" into a 6–7pm slot. v1: a real **`days_of_week`** planner knob (Mon=0…Sun=6) · explicit panel controls (day chips · time range · session length · max blocks) that drive the plan **deterministically** · the `claude -p` lane taught the new knob + worked "before 2pm"/"weekdays" examples and now *drives the controls* (each propose echoes an `applied` plan the UI snaps to; note-turns accumulate via the persisted base) · **per-key hand-vs-LLM precedence** · **schema v12** persists the prefs per-track so "weekdays before 2pm" sticks across visits + devices. Backend 709→723 · frontend 180→185 · ruff/tsc/build green; the v12 migration verified to heal a pre-v12 store. A **live** write still needs Kyle's one-time OAuth ([`STUDY_SCHEDULER.md`](STUDY_SCHEDULER.md)); until then it degrades honestly to a "connect your calendar" state. New package `app.studycal` (distinct from the SM-2 `app.study`/`store.scheduler`). Prior: **M8 — Learning Paths, the Jacobian-Lens vertical slice ✅ CLOSED (PR #136)** — the #15 green gate closed the whole slice end to end — design (#126) → Phases 1–4 (#127–129) → Designer (#130) → Plan Continue lane (#132) → Designer curation (#134) → three-trend Progress (#135) → green gate (#136). Live path quality was judged good 2026-07-22; **the slice is now live on the prod hub** — advanced from a stale ee904c9 → origin/main, frontend rebuilt, `com.homebase.server` kickstarted (health ok; `/api/paths` now carries `confidence`). **Moonshot queue: EMPTY.** Next M8 = scale the Designer beyond the one bundled fixture to the rest of the library (future). _Full history: [Changelog](#changelog-newest-first)._
...
— the epic's last line). _One honest exception, found 2026-08-11 by `/backlog-hygiene`: **live
Mermaid rendering never shipped** — `CourseDetail.tsx:910-926` still renders `.mmd` as
`<pre><code>` with an in-code note calling graph rendering a later enhancement. Everything else
on the epic's "next" list did ship._ **Arc 2 — Home Base** (kickoff 2026-07-13: the
...
items are M6's phone-side eyes-on evidence (Kyle) and the extended v1 verdict at the ~08-19
re-grade — the ~08-03 check ruled **NOT YET** (D12): the only attributed week failed the
bars, so two attributed weeks now run with `mornings_phone` ≥5 as the certifying number.
...
kanban
  ...
  next["📋 Planned"]
    rep0812["Replenish 2026-08-12 —<br/>backlog refilled via<br/>/replenish (4-lane, 69<br/>agents): 22 verified<br/>bugs over the post-07-26<br/>delta (1 high: studycal<br/>hour-24 500 · 9 medium ·<br/>12 low) + 13 refined<br/>ideas — 4 Moonshot<br/>(capture-only under the<br/>freeze: Talk Back ·<br/>Docket · Retraction<br/>Ledger · Belief Ledger)<br/>· 5 QuickWin · 4 Harden<br/>· H4 IS 07-26 bug<br/>#18's fix · fixing<br/>top-down: #1 hour-24<br/>500 ✅ PR #185 · #2<br/>Generate overwrite ✅<br/>PR #186 · #3 in-flight<br/>morning called 'thin' ✅<br/>PR #188 · #4 Habit strip<br/>fabricated '0p' for<br/>unattributed weeks ✅<br/>PR #190"]
    ...
    rep0726["Replenish 2026-07-26 —<br/>remainder RE-COUNTED<br/>2026-08-11 via<br/>/backlog-hygiene: bugs<br/>42 of 47 fixed, 5 open<br/>(#6 #15 #17 #18 #19 —<br/>all S, none<br/>test-covered; #11 + #16<br/>fixed 08-12) · ideas 2<br/>open (roster-entropy ·<br/>feed-the-vault) · 4<br/>moonshots parked/gated<br/>D7-D10 · Arc C SHIPPED"]
  ...
  later["🧊 Later / parked"]
    agentgate["Agent Gate — PARKED at<br/>its 07-27 gate<br/>conversation (D7):<br/>revisit after the<br/>extended v1 verdict<br/>(~08-19 — the 08-03<br/>check ruled NOT YET,<br/>D12) ·<br/>demand finding: Claude<br/>Code crons = a<br/>sufficient first tenant<br/>· open questions stay<br/>open for the revisit"]
...
_**Checked 2026-08-03 — ruled NOT YET (D12).** The only attributed week (07-27, Kyle's away
week) measured 3 raw / 2 phone mornings + 2 notes — under every bar — and the two 5-morning
weeks are pre-attribution build-blitz traffic. Adversarial review (F1) then showed 07-31 and
08-01 rendered no brief at all (all-8-topic API errors; 18/21 mornings in the full window
rendered — the original "21/21" counted day-directories); those mornings served the last
renderable day's brief (07-30 on both — F11), and opens still counted toward the metric (visits
stamp the visit day — F8), so only 5 of the week's 7 mornings offered a fresh brief. Extended: two attributed weeks (08-03 → 08-16),
verdict at the ~08-19 re-grade, certifying on **`mornings_phone` ≥5** with a two-step supply
check (readable mornings counted first; a week with <5 readable **and** phone <5 is a supply
failure that extends the window; phone ≥5 despite outages meets the bar and Kyle decides
whether to bank it) + ≥3 notes/week (a
saved chat answer counts once; `mac-localhost` reads certify as zero — accepted under-count,
phone-only certifies; `brief_notes` has no source guard — accepted risk, no session/agent
writes notes to the live server before the verdict). The testimony criteria are tallied live
over the window — Kyle notes each time a significant event reaches him from elsewhere first,
and whether the Google News/Reddit foraging actually stopped. Moonshot freeze + Agent Gate
park hold until the verdict._
...
- **Agent Gate** ([ideas/agent-gate.md](ideas/agent-gate.md)): PARKED at its 2026-07-27 gate
  conversation (decision D7) — revisit after the extended v1 verdict at the ~08-19 re-grade
  (the ~08-03 check ruled NOT YET, D12), whose verdict on the
...
| [`bug-hunt/2026-08-12-post-0726-delta.md`](bug-hunt/2026-08-12-post-0726-delta.md) | Post-delivery/archive/studycal-v1.x verified bug audit over the post-07-26 delta — 22 findings, ranked, triage-only |
| [`backlog-hygiene/2026-08-11.md`](backlog-hygiene/2026-08-11.md) | Hygiene run: bug/idea re-count vs the code, PR dispositions, the sweep supply + habit demand findings, and the Arc C pick |
| [`ideas/`](ideas/) | Vision docs for captured brainstorm ideas (replenishes 2026-07-19 · 07-26 · 08-12) |
...
| [`PORTFOLIO_COURSE_REFRESH_PLAN.md`](PORTFOLIO_COURSE_REFRESH_PLAN.md) | **Research-portfolio course refresh** plan (2026-08-04, corrections 2026-08-05) — regenerate the `research-portfolio` course in place to cover all 8 projects + every repo's research paper. **✅ EXECUTED 2026-08-05** — 5 modules / 16 lessons / ~8h, `validate` ok with zero warnings, live-verified. Kept as the record of the decisions and the pre-flight findings |
...
- **Two negator classes, split on whether the word can also be spent elsewhere in the note.** `no`/`not` are `_EXCLUDE_RE` day-exclusion triggers *and* carry an availability reading, so they flip a bound only across an allow-list of scheduling nouns. `nothing`/`never`/`none` are neither, so they read any ≤2 words — minus any word that opens a bound of its own or starts with a digit, which would otherwise swallow a range's opener into a zero-width window.
- `_apply_overrides` now repairs an inverted window by moving the edge the note did **not** name, so an end-only note can't be repaired back into the band it excluded. `by` stays out of the negated-start set ("not by 5pm" isn't a start phrasing). Two unnamed cases fall out too: "not until 3pm" (was `{15,15}`) and "nothing before 2pm" (a full inversion).
- **RED re-proved by base replay: 24 of the 27 touched tests fail at `3f35a80`.** The other three **pass at base** — each guards a regression a draft introduced, so they are non-regression pins, not REDs; one of the twenty-four is a pre-existing test **tightened** here. Ten mutations run **one at a time** against the final 72-test set: withholding → 13 dead · blanking → 12 · scan order → 10 · negation set → 10 · whole-note drop → 7 · named-window-as-time → 5 · note-level guard → 3 · clause scoping → 3 · gap guard → 1 · repair → 1. The clause-scoping mutation killed **zero** on an earlier sweep and exposed an unpinned shape; a pin was added and it now kills 3. **A live-LLM leak surfaced too:** one router test had begun passing by calling the real `claude` CLI once its note started falling through to the fallback — it now wires the failing fake and pins the honest degrade. Backend 929→955 · ruff green.
- **The review found sixteen should-fix defects in the fix**, fourteen of them introduced by the drafts, over 9 rounds — a day-exclusion's negation hijacking the next time trigger · an allow-list too narrow for ordinary wording · a gap that swallowed a range opener · that guard then re-inverting ordinary prepositional phrases · and a negated range applied verbatim. **The structural fix that ended the cycle:** a `nothing|never|none` no negated scan could read now withholds the window entirely, because a gap miss produced the *backwards* answer, not no answer — and the guard itself needed two more rounds, to check per occurrence (one readable clause was vouching for an unreadable one) and only within its own clause (a "nothing on Sundays" day phrase was vetoing the window stated beside it). Two nice-to-haves carried as `#192-F1`/`#192-F2`.

### 2026-08-13 — PR #190's review follow-ups carded, plus PR #185's late one

- Carded **#190-F1** (`BriefMirror` serializes `mornings_phone` with no `attributed` sibling — nothing renders it today, so it bites the next consumer, on the payload whose type advertises the number as safe), **#190-F2** (the "SW-cached payload" rationale is false: `sw.js:16`'s `OFFLINE_API` is matched by exact pathname, so `/api/brief/habit` is never cached — **pre-existing**, and the real contract argument rests on `null !== undefined`, which is unaffected), and **#190-F3** (the `store/db.py` docstring's "a week can never report a phone count the sources don't back" is a day-level promise for a week-level `any()` gate).
- #190's loop was **CLEAR** with 0 critical / 0 should-fix and nothing disputed, so no judge ran and nothing was waived — all three are carried as nice-to-haves.
- **Also carded late: `#185-F1`.** PR #185 (08-12 hunt bug #1) merged before the block-per-PR convention existed, so its one still-open finding lived only in the PR comment: hour 24 now plans, which makes a persisted `23/24` window reachable, but `PathPlayer`'s `START_HOURS`/`END_HOURS` stop at 20/22, so both dropdowns hydrate blank. That review's other finding (the `replace(hour=…)`/`fold` overstatement) was fixed twice in-PR and closed VERIFIED, so it isn't carried.
- Docs-only, non-behavioral: adversarial review skipped per the escape hatch.

### 2026-08-13 — 08-12 hunt bug #4: the Habit strip fabricated "0p" for unattributed weeks (PR #190)

- `brief_habit_weeks` had no equivalent of `mirror.py`'s `attributed` gate, so **"no source column data" and "measured zero phone reads" collapsed into the same integer 0**. Weeks whose `brief_visits` rows all predate the v14 `source` column rendered `5m / 0p` — a claim `mirror.py`'s own comment calls "a fabricated accusation, not a measurement", made by the sibling surface reading the same rows.
- **Both cases are live, which is why the gate is per-week rather than blanket.** On today's store 07-13 and 07-20 are all-NULL (**false** zeros) while 08-03 is attributed with genuinely no phone reads (a **true** zero). Hiding every zero would suppress exactly the fade signal v14 exists to catch, so the measured zero keeps rendering and only the unmeasurable one is withheld.
- **Contract: `attributed: bool` per week, not `mornings_phone: Optional[int]`** — `mirror.py` already represents this as a separate boolean (one concept, one shape, no drift between the two readers); it is additive, so the existing `[0, 0, 0, 2]` pin stays valid unchanged; and a JSON `null` would **not** be caught by HabitStrip's `!== undefined` guards (`null !== undefined` is `true`), making the Optional shape the more dangerous one, not the cheaper one.
- **The bug card misidentified the pinning test.** `test_visit_source.py`'s `[0, 0, 0, 2]` was correct all along — its 07-06 week seeds a `DEV` row, so it was attributed. The test that actually pinned the defect was `test_brief_habit.py::test_week_bucketing_distinct_days_and_window`, whose comment described the zero as "unattributed, not 'never read on the phone'" while asserting exact dicts that had no way to say so.
- **RED re-proved by replaying the final tests against base `af7f230`:** 5 fail there — 2 new backend, 2 new frontend, plus that extended exact-dict pin, which is labelled an updated pin rather than counted as a third RED. Mutation check in isolation: forcing `attributed` always-true is killed by 2 tests.
- **v1 criterion untouched** — `mornings` still counts every source, and the weeks certifying the ~08-19 verdict (08-03, 08-10) were already attributed, so the verdict was never at risk; only the July readout was. Backend 928→929 · frontend 266→268 · ruff/tsc green.

### 2026-08-13 — PR #188's review follow-ups carded + the fix deployed to the prod hub

- Carded **#188-F1** (`thin` measures dollars as a proxy for completeness; no `judged`/`pending` marker on `BriefRunDay`) and **#188-F2** (`missing_days` renders amber "no sweep: today" every morning before ~06:04 — the very defect #188 fixed, on the sibling signal). #188's F3 was upgraded to should-fix and fixed pre-merge, so it isn't carried.
- **#188-F1 is carded better than the review produced it, on Kyle's challenge.** He questioned the "half-failed morning" reading of `2026-08-09`, suspecting the off-season roster pause. The ledger: the pause is real but lands **between 08-11 and 08-12** (9 topics / $19.48 → 6 / $9.73 → 6 / $9.09), so it explains those days, not 08-09 — which ran `st-louis-blues` while still active, at catch-up hours, after a two-day outage.
- **The reframe:** 08-09's cost *per topic* is **$1.23** vs a $1.51 median — identical to 08-06's **$1.24** on a complete 9-topic day. The topics that ran ran normally; **six of nine never ran**. Cost is a lossy proxy for completeness both ways, and `topics` is already in the payload, so no roster coupling is needed — which also dissolves the pause-flag objection raised in that PR's triage.
- Banked from #188's median change: 08-09 **is** now flagged at `days=14` ($3.69 < $3.97) where the old code missed it ($3.38). It still escapes at the default `days=7`.
- **Deployed.** The prod hub was 2 commits behind with `dist/` dated **Jul 27** — serving a stale frontend for over two weeks (PR #186's `NotebookCard` fix included). Fast-forwarded to `12329d5`, rebuilt, `com.homebase.server` kickstarted; health 200, served asset matches the new build, live `runs/summary` confirms today unflagged.

### 2026-08-13 — 08-12 hunt bug #3: the sweep strip called the in-flight morning "thin" (PR #188)

- `runs_summary` computed `thin` over **today**, whose `cost_usd` is a partial sum — `sweeps/envelope.py` appends one row per topic **as each run finishes**. Replayed minute-by-minute against the live ledger (181 rows over 24 days at the time of the replay), the false "thin or half-failed" chip fires at **06:04 CT** on this morning's real sweep and clears only when the second topic lands, on the one strip whose entire job is honest trust reporting.
- Fixed with the acceptance's first option: **only days strictly older than `end` are judged**, excluded from both the median and the flag. Option two (gate on completeness) was ruled out on evidence — the ledger has **no completion sentinel**, and the only end-of-sweep artifacts (`brief.mp3`, `brief.chapters.json`) come from the **best-effort** audio step allowed to fail without failing the sweep, so `thin` would have come to depend on TTS.
- **Excluding today from the median is the second half of the same defect**, not a bonus: a $0.50 in-flight day pulls a $9.00 norm to $2.00 and silently **un-flags two genuinely thin past days** — the exact masking the flag exists to prevent.
- **A green test pinned the bug.** `test_a_day_far_under_the_norm_is_flagged_thin` passed `today=` the very day it asserted thin; rewritten to put the tell on a past day. It passes at base, so it is labelled the non-regression pin it is rather than counted as RED.
- **RED verified by replay against base `c0b5505`** (both new tests fail there), and each mutation-checked **in isolation**, failing only for the half it pins. A first combined mutation run mis-attributed its output; it was re-run rather than trusted.
- **Accepted cost:** a genuinely half-failed sweep today isn't chipped `thin` until tomorrow — the `latest` line still reports today's topics, duration, cost and errors, so it stays visible. Backend 926→928 · ruff green.

### 2026-08-13 — PR #186's four review follow-ups carded (docs-only)

- The nice-to-have findings from PR #186's pre-merge adversarial loop were living only in a PR comment. Carded so they survive: **#186-F6** (422 copy prints the server's absolute filesystem path; its test asserts a string the server never emits) · **#186-F3** (single-slot `.bak` — a second confirmed replace destroys the file the backup exists to save) · **#186-F4** (a failed `.bak` copy loses a paid generation as a 500 the "never a 500" docstring denies) · **#186-F5** (409 detail names a param no client can send).
- Numbered **by PR + finding**, not by a bare integer. The three hunts' `#2`s and `#3`s already collide — the 07-26 and 08-12 hunts each have a `#2` — so a fourth bare series would compound a problem that has already cost real cross-reference confusion.
- No code touched; the merge brief states the docs-only review skip.

### 2026-08-13 — 08-12 hunt bug #2: ✨ Generate could silently overwrite the hand-authored path (PR #186)

- The only finding whose worst case was **irrecoverable**. `generate_path` called `write_path_file` with no existence check, and that function is tempfile + `os.replace` — atomic, but atomically *destructive*. The victim is real: a **37-step hand-authored** sidecar in the gitignored `backend/data/paths/`, with no copy anywhere, that the Designer cannot reproduce (neither `video` nor `mindmap`, capped at 8 audio / 3 quiz — every regeneration strictly lossy).
- The trigger needed no user error. `api.path()` resolves null **only** on a 404 and rethrows the rest, precisely so a caller can distinguish "absent" from "broken" — and `NotebookCard` threw that away with `.catch(() => setPath(null))`, so a network blip or a 422 rendered NoPathState, whose one control is an **unconfirmed** Generate. The sharpest case inverts the danger: a *malformed* file 422s the GET, making the button most likely to appear over exactly the file only a human can repair.
- Guarded at **both** layers, because they fail independently. Policy: `generate_path` takes `replace: bool = False` and **409s** when `path_file(notebook_id)` is non-None — the examples ∪ user union, so shadowing a bundled example is deliberate too — checked **before** `compose_path`, so a refusal costs neither a minute nor a cent. Mechanism: `write_path_file` `shutil.copy2`s any existing destination to `<id>.json.bak` **inside its try**, so a failure to preserve aborts the write instead of destroying it; every future caller inherits the escape hatch, not just today's endpoint. `.bak` is invisible to `_path_files` (it globs `*.json`), pinned by its own test.
- Client half: the card carries the thrown error rather than discarding it. A **422** — the file is on disk and unparseable, so the state is permanent and hand-fixable — names itself with the parser's own detail; only a genuinely transient failure gets transient wording. (Review F1: the first pass said "Couldn't load … just now." for every failure, asserting transience about a permanent state — the same class of surface PR #182 fixed. No 422-only Generate affordance was added: the pre-fix "recovery" was destructive by definition, and a confirm-gated destructive button exceeds the finding's acceptance.)
- Verified **read-only** that `path_file` discovers the real 37-step sidecar, so the guard covers it; the endpoint was never pointed at real data. Nothing sends `replace=true` — the confirmed-replace lane is API-only by design, as a confirm affordance on the button exceeds the finding's acceptance. Distinct from known-open 07-26 #6 (stale `path_step_progress` rows on regen), deliberately not folded in.
- **RED verified by replay, not asserted.** The final test files were run against the base commit `580372f`: **8 of the 9 fail there** (6 backend · 2 card); the ninth, `test_write_path_file_first_write_leaves_no_backup`, passes at base and is labelled the non-regression pin it is. The first pass claimed "RED on all 8" — review **F2** disproved it with the same replay, and it was wrong the same way PR #185's overstatement was.
- The two backup-discovery pins were **mutation-checked**: widening `_path_files`' glob from `*.json` to `*` makes both fail. That check is why the manifest pin was also strengthened — it had filtered for `"bak"` in the id, but a leaked backup enters the union as `<id>.json` (the stem of `<id>.json.bak`), so the mutation slipped past it; it now asserts the exact id. Review F2's original target — the endpoint test — was genuinely vacuous and now asserts through `GET /api/paths`.
- Follow-ups from review (nice-to-have, not fixed here): the `.bak` is a single slot, so a second confirmed replace overwrites it (**F3**); a failed `.bak` copy aborts a paid generation as a 500 the endpoint's "never a 500" docstring doesn't acknowledge (**F4**); the 409's detail names a param no client can send (**F5**).
- Backend 919→926 · frontend 264→266 · ruff/tsc green.

### 2026-08-12 — 08-12 hunt bug #1: the studycal hour-24 propose-500 (PR #185)

- Root cause is a domain mismatch at the config → planner seam, not a bad input: `day_end_hour` is an **exclusive** end that all three callers clamp to **1..24**, and the inverted-window repair `min(24, day_start_hour + 1)` manufactures 24 outright — but `_local_window` was `day.replace(hour=hour, …)`, whose domain is 0..23. A 23:00 start (or an explicit `day_end_hour: 24`) raised `ValueError: hour must be in 0..23` as a hard 500.
- Durable, not transient: `set_study_prefs` runs **before** `plan_sessions`, so the unrepresentable window was persisted first and every later propose — even with an empty body — 500'd on the stored prefs, which the panel's 6–20 / 7–22 selects couldn't dial back out of.
- Fixed at the receiving end: `_local_window` now offsets from local midnight (`replace(hour=0…) + timedelta(hours=hour)`), the form the sibling `_events_in_window` already used, so hour 24 means "closes at local midnight". Narrowing the clamps to 23 was rejected — it would silently eat the last hour of a legitimate "study until midnight" window and leave a 23:00 start unrepairable.
- **0..23 behaviour is byte-identical** — verified over 168 hour × day combos spanning both 2026 DST transitions (timedelta on an aware datetime is wall-clock arithmetic, so offset resolution is unchanged).
- No migration for already-persisted 23/24 prefs: once 24 plans, those are a legitimate 11pm–midnight window, not poison. Pinned by a router test that re-proposes with an **empty body** against the stored prefs and asserts the same `applied`.
- Test-first: three tests RED at `ab9dc58` (planner `ValueError`, and both router routes) → green on the one-line fix. Backend 916 → **919** · ruff green.

### 2026-08-12 — Backlog replenished: 22 verified bugs + 13 ideas across four lanes

- `/replenish` (Standard preset, 69 agents ≤ the 70 cap): bug-hunt over the post-07-26 delta (7 finders incl. `silent-failure-hunter` + `contract-reviewer` → verify-each, default-refuted → synthesis) ran beside 18 blind lenses; 46 deduped candidates faced each mode's two opposing critics + Soul-Keeper; 13 picks got an opus steelman pass each.
- Bugs: 27 confirmed, merged to **22 ranked** ([report](bug-hunt/2026-08-12-post-0726-delta.md)) — top of the list: studycal `hour=24` 500 with durable bad state (high), path-sidecar overwrite with no backup, the trust strip flagging the in-flight morning as thin, habit-strip "0p" fabrication on unattributed weeks, inverted negated time prefs. Recurring theme: honesty-of-readouts regressing on new surfaces.
- Ideas: **13 captured** as vision docs + stubs — Moonshots (freeze-respecting, capture-only): Talk Back (4-lens convergence), The Docket, The Retraction Ledger, Belief Ledger (wildcard); QuickWins: delivery receipt (3-lens), item anchors, listening-counts-as-a-morning, habit supply denominator, thin-day honesty in delivery; Hardens: don't-send-half-a-brief (merged pair), canonical-root pin, uncounted-morning replay, untrusted-lane invariant (= the 07-26 hunt's designer-prompt bug #18 fix).
- Honorable mentions (22 gate-clearing, quota-cut) live in the run transcript; Harden's reconcile-on-read folded into bug #11 as its fix.

### 2026-08-12 — iMessage brief becomes text + tap-to-play tailnet link (PR #183)
...
- The osascript test assertion is inverted (`[-, handle, caption]`, no file argv), so a
  regression back to the silent file-send fails loudly; 13 delivery tests total (12 subprocess
  + 1 `build_html` unit test). Review round 1 hardened the claim end to end: the audio endpoint
  now serves `Content-Disposition: inline` so the tap plays instead of downloading (F1), a
  link-less send says so in stdout + the ledger (`text only (no base_url)`) with a stderr
  warning instead of claiming a link (F2), and `base_url` was set in the live gitignored
  `delivery.json` same-day; the link path's live verify (`--force` re-send against the
  kickstarted post-merge server) runs at rollout, result recorded in the PR (F3). Value stays
...
- **Bug #16 — Overnight approve/discard was check-then-act (PR #182, RED→green).** The docstring
  ...
  decision", but the check was an unguarded read: resolve → `add_brief_note` → `append_status` are
  three steps on FastAPI's threadpool. The **note** is the durable damage — the ledger self-heals
  on read (`load_queue` honours the first status row only) and `brief_notes` has no uniqueness
  constraint, so a doubled note joins the served brief and `/notes` as a real, separately-deletable
  row. Fix: a module-level `_overnight_lock` — the same shape as `_sweep_lock` twenty lines up in
  the same file, for the same reason (one always-on server is the choke point, so a lock, not a
  lockfile) — wrapping all three steps on **both** verbs. `_resolve_overnight` now documents that
  the single-shot promise belongs to the lock, not to itself.
- **Its client half.** `Brief.tsx` disabled Approve on `fromCache` only, with nothing between tap
  and response — so an impatient double tap on a slow phone fired twice and surfaced "Couldn't
  save" on a draft that had saved fine. A per-proposal `overnightPending` now kills **both** verbs
  in flight (the mind-change is as dangerous as the double tap), cleared in `.finally()` so a
  failure re-arms rather than stranding the draft, and scoped per card so a sibling stays actionable.
- **Bug #11 — Progress blanked on the one rejection it couldn't survive (PR #182, RED→green).** The
  `Promise.allSettled` degrade was real and its intent comment said so, but **both** body branches
  gated on `data`, set only when `api.progress()` fulfils, and the banner needed all three core
  calls to reject. Net: progress alone rejects → header over nothing, no banner, while review,
  reflections and paths had all loaded and sat unreachable behind the gate. The body now gates on
  `hasAnything`; the four blocks that actually read `/progress` null-guard individually
  (`ThreeAxisBand` is the only one that genuinely needs it, so it alone hides); the banner tracks
  progress's **own** rejection and says "the other sections below still loaded" when they did. The
  "No attempts yet" empty state is suppressed under a banner — with a failed endpoint it's a claim
  the page can't make.
- **The RED was real, and deterministic rather than hopeful.** A shared `_park_after_resolve`
  helper holds every tap that clears the resolve read until a *second* one clears it too:
  unguarded, both arrive and the window is provably open; guarded, the second is still on the lock,
  so the barrier breaks on its own timeout and the first proceeds alone — the same invariant is
  asserted either way and only the guard decides whether it holds. Verified failing **5/5 runs** at
  `28a9dbd` ("a double tap wrote 2 real notes"), passing 3/3 after. Precedent: the calibration
  ledger lock test (PR #158). A crossed-verb approve-vs-discard race is pinned too, and Progress's
  RED was `Unable to find role="heading" and name "Reflections"` with the other three calls fulfilled.
- **Adversarial review (round 1: 0 critical · 1 should-fix · 4 nice-to-have).** The should-fix was
  the stale-tab half of the very phrase the fix quotes: a 409 rendered "Couldn't save" on a note
  that had saved, and left live verbs on a resolved card. It now reads "Already resolved on another
  device — reload to see it" and retires the verbs. The reviewer independently reproduced the RED by
  neutering `_overnight_lock` from an out-of-tree pytest plugin, and confirmed `serve/run-server.sh`
  runs a single uvicorn with no `--workers`, so a module lock is a valid guard for the deployed
  shape. Four nice-to-haves carried as follow-ups — notably that the lock closes the *concurrency*
  window but not the *failure* window (note commits, `append_status` raises, a retry doubles the
  note): the optional half of #16's own acceptance, now recorded as deliberately skipped rather than
  quietly implied fixed.
- **Counts.** Backend **914 passed** · frontend **264 passed** — this PR contributes +3 backend / +5 frontend; the branch also inherits PR #181's 11 delivery tests via the rebase onto `c3606ac` · ruff + tsc green. Bugs
  now **42 of 47 fixed, 5 open** (#6 · #15 · #17 · #18 · #19 — all S-sized, none test-covered).
  The 07-26 hunt is 19 of 24.

### 2026-08-11 — The brief gets delivered: email + iMessage after every sweep (PR #181)
...
- **`/backlog-hygiene` full run** — brief at [`backlog-hygiene/2026-08-11.md`](backlog-hygiene/2026-08-11.md).
  ...
  item verified against the code at `5a72ea5`, not against its own text: **bugs 40 of 47 fixed,
  7 open** (#6 · #11 · #15 · #16 · #17 · #18 · #19 — all S-sized, **none test-covered**); **ideas
  2 open** (`roster-entropy` · `feed-the-vault-ritual`); 4 moonshots parked/gated at D7–D10.
  **#17's shape enlarged by PR #173** — `video`/`mindmap` shipped frontend-only, so both render as
  first-class rateable steps while `_TYPE_FOR_KIND` still knows 4 kinds and treats any unknown one
  as glue.
- **Doc truth:** episode-review had claimed "In progress" on a branch deleted ~2 months earlier
  (shipped `c0a030b`, 2026-06-05); the course epic's "Next (M2+)" list was 4-of-5 shipped — **only
  live Mermaid rendering remains** (`CourseDetail.tsx:910-926`), which this file had also
  over-claimed. PRs **#145** and **#125** closed as superseded — #145's merge would have been
  **−9434 lines**, deleting the Wiki, LICENSE, `visit_source.py` and ~50 test files.
- **Finding 1 — supply.** The 06:00 sweep failed 4 of the last 12 mornings on two separable causes:
  a **weekly-quota wall recurring as Fri+Sat pairs exactly 7 days apart** (07-31/08-01, 08-07/08-08
  — next due **08-14/15, inside the graded window**) and mid-response connection drops burning
  ~$1–2/topic. `sweep.sh` has **no retry** and cannot distinguish retryable-after-reset from
  malformed-JSON. **Failed-topic spend is never ledgered** — 08-09 records $3.69 against **$12.38
  spent**, so PR #163's readout under-reports 70% on the worst day and its `thin` flag calls that
  day cheap; the heartbeat measures silence, not completeness, and logged "ok" with 6 of 8 dead.
- **Finding 2 — demand.** Week 1 of the graded window recorded **0 phone mornings** (bar ≥5) and
  **1 note** (bar ≥3); last note 08-03; 6 visit-days in the 16 since attribution began.
  Instrumentation verified healthy. Under D12's amendment the supply wobble would classify this as
  a *supply* failure and extend the window a third time. **The one input only Kyle has — whether he
  was away 08-03→08-10 — is left deliberately unanswered.**
- **Roster paused:** `celtics`, `st-louis-blues`, `kansas-basketball` (out of season; ~$7/day of a
  ~$19/day burn; reversible in one tap from Today's chips). Kyle's call at the gate.
- **Arc C picked** for the next session: #16 (Overnight approve/discard is check-then-act — a
  double-tap can double a real note, against a docstring promising it cannot) and #11 (Progress
  blanks when one of four calls fails, against its own intent comment). Docs + roster only here —
  **no code was written by this run.**

### 2026-08-05 — Research-portfolio course REGENERATED over all eight projects

- Executed [`PORTFOLIO_COURSE_REFRESH_PLAN.md`](PORTFOLIO_COURSE_REFRESH_PLAN.md) end to end
  (local Opus 5 session). The `research-portfolio` course went from **6 projects / 5 modules /
  10 lessons / ~5h** to **8 projects / 5 modules / 16 lessons / ~8h**, regenerated **in place**
  (same slug, same URL, progress was 0/11 so nothing was preserved). One approval gate, then
  autonomous authoring by a per-module subagent fan-out in two waves.
- **What's structurally new:** the shared spine teaches the **two-lane** framing and the **anchor
  ladder** (published paper → technique → own recorded result → original question) and adds *why a
  null needs its own power argument*. Lane 1 runs the J-lens lineage as one arc — dim-stage →
  mute-map → hush-gauge, the last split across two lessons for its five-gate ledger. Lane 2 splits
  the reproductions that landed from the nulls, and **blind-cite's reversal is the course's
  centerpiece defense story**. All eight repos' **papers + presenter packs** are wired in as
  `reading` (35 readings, every URL verified 200 before authoring).
- **Pre-flight found four drift items, and the plan's standing rule (the repo wins) was applied:**
  `blind-cite`'s and `hush-gauge`'s portfolio cards were both stale, and so were the front-matter
  docs that inherited them. The course was authored from the **repos and papers**, never the cards
  — then the cards themselves were fixed under their own PRs rather than left wrong
  (ksdisch/portfolio#11 = D14; ksdisch/hush-gauge#20, whose README was a milestone behind at
  M0–M3). Also found: all eight repos now ship a paper + presenter pack, and four NotebookLM
  artifacts created 08-04 are absent from the sidecar (reported, not touched).
- **NotebookLM, at Kyle's request at the gate:** `hush-gauge` onboarded (card + raw README + paper
  + presenter pack, the last two read from `origin/main` blobs) and **S2 Ep 4 — hush-gauge** audio
  + an 8-question quiz generated, both scoped by `source_ids` to the paper rather than the stale
  card. `mute-map` needed nothing — `S2 Ep 1 — mute-map` (08-01) postdates its M4 close-out.
  Sidecar manifest 23 → 27 sources.
- **Verification:** `cli write` → **ok:true, zero errors, zero warnings**; 8 stale material files
  from the six-project version deleted and re-validated; quiz discipline and markdown-subset
...
- Docs-only. At the calendar-gated ~08-03 check, the only attribution-clean week (07-27 —
  also Kyle's away week) failed every measurable bar: 3 raw / 2 phone mornings vs ≥5, 2
  notes vs ≥3. The two raw-passing weeks (5 · 5) predate #159's attribution and are July
  build-blitz traffic — certifying them would stamp build exhaust as a reading habit, the
  exact failure the builder-vs-reader premortem predicted.
- Kyle's ruling: **NOT YET** — two attributed weeks (08-03 → 08-16), verdict lands at the
  ~08-19 re-grade (two gates collapse into one). Certifying number: **`mornings_phone` ≥5**;
  notes ≥3/week (a saved chat answer counts once). Testimony criteria (events-first ·
  foraging) tallied live over the window rather than reconstructed.
- Moonshot-lane freeze + Agent Gate park (D7) hold until the extended verdict.
- Adversarial review round 1 corrected the record before merge: **supply side was NOT clean**
  — "21/21 days" counted day-directories; 07-24, 07-31, 08-01 rendered no brief (all-8-topic
  API errors), two inside the graded week — which left only 5 of 7 mornings with a fresh brief
  (F1; the metric ceiling stayed 7, since visits stamp the visit day — F8). The ~08-19 verdict
  therefore runs a two-step supply check: readable mornings counted first; <5 readable **and**
  phone <5 extends the window; phone ≥5 despite outages meets the bar (Kyle banks it or not). The
  phone metric's error model is recorded both ways (`mac-localhost` certifies as zero — F3),
  the Agent Gate unpark trigger repointed in its vision doc + BACKLOG (F2), and the notes
  bar's missing attribution recorded as an accepted risk with a no-agent-notes convention
  (F7). Suites at the check: backend full pass + frontend 259/259.

### 2026-08-02 — Research-portfolio study path supersedes the S1 fixture + video/mindmap kinds (PR #173)

- The interview-prep notebook's hub path, broken by the 2026-08-01 refresh (3 deleted
  study-aid ids) and blind to 24 of the notebook's 32 artifacts, is **superseded** by a
  hand-authored 37-step phased path over all 32: Orient → S1 lap (soft-gated quiz per
  episode, 6/8 honor-system; videos interleaved) → Grilling I → S2 lap → Debate/Critique/
  Grilling II → drill. Kyle picked supersede + soft gates from three proposals (**D11**).
- Player learns `video`/`mindmap` step kinds — D6-honest: audio spine, videos supplementary
  under their real kind/artifact_type (decision 12 amended in `docs/ideas/learning-paths.md`).
  Path data is gitignored user data, validated type-aware 32/32 against the hub catalog and
  already live; frontend tests 258→259 · tsc green.
