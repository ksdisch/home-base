...
kanban
    ...
    lh15["LH Phases 1–5 — SPEC build<br/>order: catalog · quiz player ·<br/>progress · review queue ·<br/>custom topics"]
    lh6["LH Phase 6 — SM-2 SR core +<br/>daily study plan + reflections<br/>journal"]
    lh7["Courses M1 — course sidecars,<br/>read+track API, Courses UI,<br/>/build-course"]
    lh7m2["Courses M2 — course quizzes in<br/>the quiz player + per-course<br/>SM-2"]
    lh7m3["Courses M3 — generation at<br/>depth: project/capstone +<br/>tracked rubrics · course<br/>what-to-do-next · skill<br/>fan-out at depth"]
    lh7fc["Courses M2 remainder —<br/>flashcard review UI: dedicated<br/>session page + per-card SM-2 +<br/>due chips"]
    lh7m4["Courses M4 — NotebookLM<br/>enrichment in-flow: catalog<br/>cross-link cards on course<br/>pages + gated link-or-generate<br/>skill flow"]
    hbm1["HB M1 — the brief page: Today<br/>route, /api/brief, visit log ·<br/>PR 36"]
    hbm2["HB M2 — full 8-topic roster +<br/>inline notes + Your-learning<br/>strip · PRs 38-39"]
    hbm3["HB M3 — hands-off: launchd<br/>06:00 schedule + dedup labels<br/>+ cost ledger · PR 43 · first<br/>unattended fire verified<br/>2026-07-16"]
    hbm4["HB M4 — audio brief: 5-min<br/>Kokoro MP3 after every sweep +<br/>Today player · PR 45"]
    hbm5["HB M5 — chat with the brief:<br/>per-item Ask · grounded<br/>answers, subscription lane, no<br/>web · save-as-note · PR 47"]
    hbm6["HB M6 — mobile: one-port serve<br/>+ LaunchAgent · sw.js v2<br/>cached last brief · bottom tab<br/>bar + Today/Notes pass · PRs<br/>55-56 · Mac-side live verify<br/>clean 07-18, phone proof<br/>pending Kyle"]
    hbm7["HB M7 — news mode: RSS<br/>category shell · news_events<br/>signal log + card feedback ·<br/>For You decaying-profile<br/>ranker w/ search-feed reach ·<br/>topic scout → one-click roster<br/>adds · PRs 58/60/62/63 +<br/>polish 65-67 · live e2e proof<br/>clean 07-18"]
    hbm0["HB M0 — sweep-quality week:<br/>CLOSED 07-19, verdict PASS ·<br/>zero fabrications all week ·<br/>AI sweep prompt tuned · grades<br/>+ audit in M0-sweep-grades.md"]
    cm5["Courses M5 — authoring loop in<br/>the hub: objectives editor ·<br/>reorder w/ id pinning ·<br/>regenerate on the claude lane<br/>w/ validate-or-rollback · zip<br/>export · epic complete 07-19"]
    bugp1["Bug fixes P1 — all 5 medium<br/>07-19 hunt findings fixed<br/>07-19, one PR each,<br/>test-first: blank-brief PR 73<br/>· promptless scout add PR 74 ·<br/>frozen news category PR 75 ·<br/>UTC streak days PR 76 ·<br/>dev-vs-prod port clash PR 77"]
  doing["🔄 In progress"]
    hbm6proof["HB M6 remainder — phone<br/>eyes-on evidence: home-screen<br/>standalone · airplane-mode<br/>banner · iOS audio scrub ·<br/>reboot survival · Kyle"]
  next["📋 Planned"]
    trust["Trust + liveness cluster —<br/>ran-or-didnt banner QU12 ·<br/>sweep-trust gauge PR5 ·<br/>heartbeat outside the app PR12"]
    qnotes["Notes on News cards QU5 —<br/>cross-mode capture, serves the<br/>notes/week v1 criterion"]
  decide["⏸️ Awaiting decision"]
  later["🧊 Later / parked"]
    defer["Kickoff-deferred: ESPN ·<br/>auto-courses · alerts · public<br/>writing"]
    moonshots["4 Moonshot bets — Overnight<br/>Chief of Staff · The Mirror ·<br/>Readiness Brief · Calibrated<br/>Doubt wildcard"]
    quickwins["3 QuickWins parked —<br/>yesterdays brief QU1 · audio<br/>resume QU3 · topic chips QU4 —<br/>QU5 + QU12 already planned"]
    premortems["1 Premortem antibody parked —<br/>vault-ritual feed PR10 —<br/>heartbeat PR12 + trust gauge<br/>PR5 ride the Planned trust<br/>cluster"]
    hardens["4 Harden guards — re-sweep<br/>note detach HA2 · untrusted<br/>framing HA4 · feed-drift guard<br/>HA8 · migration snapshot HA11"]
    frictions["5 Friction smoothings — nav<br/>survival FR15 · phone sweep<br/>trigger FR2 · audio chapters<br/>FR4 · developing context FR13<br/>· undo beat FR10"]
    buglows["18 low bug stubs — ranked with<br/>fixes in the 2026-07-19<br/>bug-hunt report · BACKLOG Open<br/>section"]
```
