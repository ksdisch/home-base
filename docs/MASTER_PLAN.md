...
weeks are pre-attribution build-blitz traffic. Extended: two attributed weeks (08-03 → 08-16),
verdict at the ~08-19 re-grade, certifying on **`mornings_phone` ≥5** + ≥3 notes/week (a saved
chat answer counts once). The testimony criteria are tallied live over the window — Kyle notes
each time a significant event reaches him from elsewhere first, and whether the Google
News/Reddit foraging actually stopped. Moonshot freeze + Agent Gate park hold until the
verdict._
...
- Moonshot-lane freeze + Agent Gate park (D7) hold until the extended verdict. Supply side
  clean: sweeps 21/21 days; backend full pass + frontend 259/259 at the check.
