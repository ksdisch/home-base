# Phase 7 Plan — Courses (the course-pipeline vertical slice)

_The first milestone (M1) of the Course Pipeline epic (see `docs/COURSE_PIPELINE_SPEC.md`).
...
> Originally authored as "Phase 6" on the course branch; renumbered to **Phase 7** because
> the SR work (`PHASE6_PLAN.md`) shipped to `main` as Phase 6 first. M2 (course quizzes in the
> player + SM-2 SR) is tracked separately.
...
- Taking a course quiz *in the player* (shaped now, routed later); flashcard review UI. **← M2**
