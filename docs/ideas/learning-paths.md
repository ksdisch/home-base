...
12. **Audio overviews are the spine; video is supplementary.** When a topic has both an audio-overview season and a whiteboard/explainer video season, a path (and any course built from the same notebook) is composed around the **audio** episodes by default — video overviews are optional supplementary material (surface them in glue steps like Reflect, never as the artifact-backed spine) unless the topic is explicitly video-first. The Designer already enforces this structurally — `paths/designer.py` only arranges `audio/study_guide/flashcards/quiz` and never even sees `video` artifacts — so this decision governs **hand-authored** fixtures/paths too, which bypass that cross-check. Guarded for the shipped fixture by `backend/tests/test_paths_fixture.py`. (Rationale: the two seasons carry parallel `Ep N —` titles, so it's easy to grab the wrong ids — the original Jacobian fixture shipped the video ids mislabeled as `audio`.)
