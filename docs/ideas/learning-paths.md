...
12. **Audio overviews are the spine; video is supplementary.** When a topic has both an audio-overview season and a whiteboard/explainer video season, a path (and any course built from the same notebook) is composed around the **audio** episodes by default — video overviews are supplementary material, never the artifact-backed spine, unless the topic is explicitly video-first. *Amended by D11 (2026-08-02, `Decisions.md`): a hand-authored path MAY interleave supplementary videos as artifact-backed steps under the honest `video` kind (real `artifact_id`, `artifact_type: "video"`, a 🎬 Watch launch — as the research-portfolio path does); what stays forbidden is building the spine on video or mislabeling video ids as `audio`. Generated paths keep the pre-D11 form — the Designer still arranges only `audio/study_guide/flashcards/quiz`.* The Designer enforces the spine rule structurally and never even sees `video` artifacts, so this decision governs **hand-authored** fixtures/paths too, which bypass that cross-check. `backend/tests/test_paths_fixture.py` guards the **bundled Jacobian fixture only**; other hand-authored paths (e.g. the gitignored research-portfolio path) have no automated guard and rely on type-aware validation at authoring time (all artifact ids resolve in the catalog with types matching their step kinds). (Rationale: the two seasons carry parallel `Ep N —` titles, so it's easy to grab the wrong ids — the original Jacobian fixture shipped the video ids mislabeled as `audio`.)
