...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #192, RED→green): both halves, exactly as the acceptance names them, but the final shape is **three review rounds** past the first draft — read the design off this note, not off the acceptance. A negated bound sets the **opposite** edge through two new triggers, `_NEG_END_TRIG` (negation … `after`) and `_NEG_START_TRIG` (negation … `before|until|till|til`), because Python's `re` requires lookbehinds fixed-width and the two literals this replaces (`(?<!not )(?<!no )`) could only see the word immediately before `after`. **The load-bearing detail is that there are TWO negator classes with two different gaps**, split on whether the same word can also be spent elsewhere in the note: `no`/`not` are `_EXCLUDE_RE` day-exclusion triggers **and** carry an availability reading ("I have no meetings after 3pm" means those hours are free), so they only flip a bound across an **allow-list** of nouns naming the thing being scheduled (`more|sessions|blocks|study|studying` — `work` deliberately excluded as ambiguous); `nothing`/`never`/`none` are neither, so they read **any ≤2 words** — except a word that opens a bound of its own, or a time, which would otherwise swallow a range's opener ("nothing from 9am until 5pm" → a zero-width window). Every scan runs against a working copy with **every match blanked out** — `_parse_days`' own trick — and **repeats until its pattern is exhausted**, so no occurrence survives into a later opposite-direction scan; blanking preserves string length, so offsets stay comparable and the **leftmost match per edge wins**, which is what the two single searches did ("not before 2pm or after 5pm" still reads 2pm as the start). **The negated forms are scanned first**, because each one's trigger word belongs to the other direction. `by` was deliberately **not** added to the negated-start set — "not by 5pm" isn't a start phrasing; END's bare `by` is protected structurally by the blanking. One router change rides along: `_apply_overrides` now repairs an inverted window by moving the edge the note did **not** name, so an end-only note against a standing evening window can't be "repaired" back into the band it excluded (the structured-controls repair at `study.py:297` is untouched). Two cases the card didn't name fall out of the same pass: `"not until 3pm"` (was `{15, 15}`) and `"nothing before 2pm"` (a full inversion). **RED verified by replaying the final tests against base `3f35a80`: 12 of 13 fail there**; the thirteenth, `test_a_negation_spent_on_a_day_exclusion_does_not_also_flip_a_time_bound`, **passes at base** — it pins behaviour the base already had right and is the non-regression pin for a regression the first draft introduced, not a RED. Twelve include one **pre-existing test tightened here** (`test_exclusion_trigger_words_do_not_misfire_on_time_phrases` pinned only that no day override leaked out of "not before 2pm", leaving the spurious `day_end_hour` unguarded), labelled an updated pin. **Four mutations, each applied and reverted in isolation:** narrowing the negation set kills 4 · removing the gap kills 2 · removing the blanking kills all 5 · flipping the scan order kills all 5. **Adversarial review found 4 should-fix defects in the first draft** — three of them regressions the draft itself introduced — across 3 rounds; all fixed and re-verified. Open follow-ups: `#192-F1` (distributed "nothing before 9am **or after 5pm**" drops the second bound) · `#192-F2` (an end-only note collapses the standing window to one hour and persists it). Backend 929→941 · ruff green · frontend untouched (no contract change)._
