...
- **Size:** S
- **Added:** 2026-07-26
- _✅ shipped 2026-07-27 (RED→green — replenish small-wins item 5/9): `runs_summary()` in `app/sweeps.py` totals `.runs.jsonl` per day → `GET /api/brief/runs/summary?days=` (clamped 1–90) → a `SweepLedgerStrip` ops line + 7-day roll-up under the habit strip on Today. Zero LLM, read-only, tolerant of a mangled line. Both live anomaly shapes are surfaced by construction: the window is **calendar-dense** so a day with no rows at all (the 07-24 gap) is present as `ran: false` and named under "no sweep", and a day that ran but cost far under the window's median (the $1-2 07-23/07-25 shape) is flagged `thin` — a median over only the days that ran, so a gap can't drag the norm down and mask a thin day. `topics` counts distinct topics so a re-sweep doesn't inflate it, while both runs' cost still totals. The line says "This morning" only when the newest day that actually ran IS today; otherwise it names the date, so a stale total can never wear today's label. Idea-doc open questions decided: (1) **footer line** under the habit/trust strip, not a chip — it's an ops readout, not a badge; (2) **flag, don't just inform** — purely informational would have failed "surface visibly", but it stays a tint with no alerting (out of scope). 22 tests (13 backend + 9 frontend). **Kyle's confirmation against the real 69-row ledger is outstanding** — `.runs.jsonl` is gitignored, so this clone has no live data; the shapes are pinned with fixtures built to the exact writer schema in `sweeps/envelope.py`._
...
- **Size:** S
- **Added:** 2026-07-26
- _✅ shipped 2026-07-27 (RED→green — replenish small-wins item 8/9): `navigator.mediaSession` metadata ("Morning brief — Jul 14", artist = the chapter currently playing, refreshed on timeupdate) + play/pause/seekto/previoustrack/nexttrack handlers, plus persisted 1x/1.25x/1.5x chips (`audio-rate`, re-applied on `loadedmetadata` because `load()` resets `playbackRate`). Landed on `BriefAudioCard` — the ONE shared player after bug #20's extract — which answers idea-doc question 2 by construction: **the archive player gets the same wiring in the same pass, for free**. Question 1 decided: **the two scarce skip slots go to chapters, not a blind ±15s** — the brief has real chapters with offsets and nothing else surfaces them, while ±15s is already reachable through the scrubber that `seekto` registers. Chapter skip reuses the chips' own −2s lead, and ⏮ restarts the current chapter before stepping back (standard media behavior — a first-draft test asserted otherwise and was corrected to the real semantics). Absent `mediaSession` (iOS < 15) degrades silently; the card still works. 13 new tests, frontend 240→253. **Kyle's eyes-on lock-screen pass is outstanding** — jsdom has no Media Session API, so the whole surface is stubbed and only the real phone can confirm the OS actually renders it._
...
- **Size:** S–M
- **Added:** 2026-07-26
- _✅ shipped 2026-07-27 (RED→green — replenish small-wins item 9/9, batch complete): `search_briefs()` in `app/sweeps.py` walks `data/sweeps/*/<topic>.json` newest-day-first over headline + digest + why_it_matters, plus `brief_notes` (body + item_headline), plain case-insensitive substring → `GET /api/brief/search?q=` → a search box atop `/archive`, each hit deep-linking to `/brief/<date>`. Zero LLM, zero index, strictly read-only (one test asserts no mtime under `data/sweeps` changes). Reuses `_structured_topic` so hit `item_id`s are the same read-time anchors notes attach to. Idea-doc open questions decided: (1) **one blended list, labeled by `kind`** — two groups would make "where did I read that?" a two-place search; (2) **cap 50, but `total` reports what was FOUND, not what fit**, so a truncated result says "Showing 25 of 110" instead of quietly looking complete; (3) **archive page only** for now — a global header affordance stays unscoped. A blank query returns nothing rather than the corpus (that's a browse, and `/archive` already is one); an unparseable day is skipped, never fatal. 17 tests (12 backend + 5 frontend). Backend 880→892 · frontend 253→258. **Kyle's phone check — finding a half-remembered July item — is outstanding**; this clone has only fixture sweeps._
...
- **Size:** S
- **Added:** 2026-07-26
- _✅ shipped 2026-07-27 (RED→green — replenish small-wins item 6/9): `set_topic_paused()` beside `append_roster_topic` in `app/news.py`, sharing its `_roster_write_lock` + atomic replace (both now go through one extracted `_write_roster`) → `PATCH /api/brief/topics/{slug}` (unknown slug = 404, never a silent append; unusable roster = a named 400) → a ⏸ control on each Today chip and a dashed ▶ chip per muted topic. **`BriefResponse.paused_topics` is load-bearing, not decoration**: a paused topic produces no file, so it has no card and no jump chip — without it the phone-side mute would be one-way. Every other roster entry, and every key this code doesn't know about, is written back verbatim; a no-op toggle doesn't rewrite the file at all. Idea-doc open questions decided: (1) **an explicit control, never a long-press** — the chip row scrolls horizontally, so press-and-hold would misfire on every drag; the label keeps the jump, the ⏸ owns the mute; (2) **no confirm beat and no undo toast** — the opposite tap is right there on the same chip, which is only true *because* paused topics stay visible. 19 tests (14 backend incl. a barrier-raced concurrent toggle, 5 frontend). Backend 857→871 · frontend 232→236._
...
- **Size:** S
- **Added:** 2026-07-26
- _✅ shipped 2026-07-27 (RED→green — replenish small-wins item 4/9): a `wait_for_network` preflight between the `command -v claude` check and `./sweep.sh` — `curl -s -m 3` against `$SWEEP_NET_PROBE_URL` (default `https://news.google.com/`) every 5s for up to 90s, ticks to the existing dated `$LOG`, then **exit 69 before `sweep.sh` is ever invoked** so no topic state is written and `SWEEP_SKIP_DONE=1` lets the next wake finish the day. Cap/interval/URL are env-overridable. Five tests sandbox the wrapper with stub `curl`/`claude`/`sweep.sh`; **also verified with real curl** — a reachable host runs the sweep with zero wait ticks, an unroutable one logs the ticks and aborts 69 with the sweep never invoked. Idea-doc open questions decided: (1) **90s**, env-overridable; (2) **HTTPS**, because a captive portal can answer a plain http:// GET with a 200 but cannot complete someone else's TLS handshake — and deliberately **no `--fail`**, so any real reply (a 400 counts, as the real-curl check confirmed) proves reachability; (3) **no new notification** — the didn't-run banner + heartbeat + next-wake retry already surface it. Backend 839→844. **Kyle's live Wi-Fi-off check on the Mac is still outstanding** — the container has no Wi-Fi to toggle._
...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #186, RED→green): guarded at both layers. `generate_path` takes `replace: bool = False` and 409s when `path_file(notebook_id)` is non-None — keyed on the examples ∪ user union (shadowing a bundled example is deliberate too), and placed before `compose_path` so the refusal costs neither a minute nor a cent; it fires on a malformed file too, which is the sharpest case since a malformed path is what 422s the GET into the very state that renders Generate. `write_path_file` now `shutil.copy2`s any existing destination to `<id>.json.bak` inside its try, so failing to preserve the old file aborts the write instead of destroying it; `.bak` stays out of `_path_files` (globs `*.json`). `NotebookCard` now carries the thrown error — `api.path()` already resolves null only for 404 and rethrows the rest — so a **422** (file on disk, unparseable: permanent and hand-fixable) names itself with the parser's detail while only a genuinely transient failure gets transient wording (review F1). Verified read-only that the real 37-step hand-authored sidecar is discovered by `path_file`, so the guard covers it. **RED verified by replaying the final tests against base `580372f`: 8 of 9 fail there** (6 backend · 2 card); the ninth is the non-regression pin. Both backup-discovery pins mutation-checked against a widened glob. Nothing sends `replace=true` — the confirmed-replace lane is API-only by design (no confirm affordance was added to the button; that would exceed acceptance), and review F5 confirmed the 409 is unreachable from the UI by construction, so it is defense-in-depth for the API lane. Open follow-ups from review: single-slot `.bak` (F3) · a failed backup 500s a paid generation (F4) · the 409 detail names a param no client can send (F5)._
...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #188, RED→green): took the first acceptance option — `runs_summary` now judges only days strictly older than `end`, excluding the current day from **both** the median and the `thin` flag. The second option was ruled out on evidence, not taste: the ledger carries no completion sentinel (`sweeps/envelope.py` appends one row per topic as each run finishes), and the only end-of-sweep filesystem artifacts are `brief.mp3`/`brief.chapters.json` from the **best-effort** audio step that is explicitly allowed to fail without failing the sweep — gating `thin` on those would make the trust readout depend on TTS, and a roster-size comparison would couple it to `topics.json` and its pause flags. "Strictly older than `end`" is the only completeness predicate actually knowable here. Excluding the current day from the **median** is the second half of the same defect, not a bonus: an in-flight day is not a sample of what a day costs, so it biases the norm toward zero — demonstrated at $0.50 pulling a $9.00 norm to $2.00 and silently un-flagging two genuinely thin past days. **RED verified by replaying the final tests against base `c0b5505`: both new tests fail there** (`test_the_in_flight_morning_is_not_flagged_thin`, `test_a_partial_today_cannot_drag_the_norm_down_and_mask_a_thin_day`); each was mutation-checked in isolation and fails only for the half it pins (median-only mutation → the masking test; flag-only mutation → the in-flight test). The pre-existing `test_a_day_far_under_the_norm_is_flagged_thin` **pinned the bug** — it passed `today=` the very day it asserted thin — so it was rewritten to put the tell on a past day; it passes at base too, making it a non-regression pin rather than a RED test. Replaying the live ledger (181 rows over 24 days at the time of the replay) minute-by-minute through this morning's real sweep (06:04→06:17 CT), the false `thin` chip that fired at 06:04 is gone at every cutoff. Backend 926 → 928. **Accepted cost:** a genuinely half-failed sweep today is not chipped `thin` until tomorrow — the `latest` line still reports today's topics, duration, cost and errors directly, so it stays visible, just not with that word._
...
- **Size:** M
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #190, RED→green): took the **`attributed: bool` per week** branch of the acceptance, not the `Optional[int]`. Three reasons, in order: (1) `mirror.py` — the sibling reader of the very same `brief_visits` rows — already carries this as a separate boolean, so one concept keeps one shape and the two surfaces can't drift; (2) it is additive — `mornings_phone` keeps its type and the existing `[0, 0, 0, 2]` pin in `test_visit_source.py` stays valid unchanged (its 07-06 week seeds a `DEV` row, so that week was **attributed** and its 0 was a true zero all along — the bug card's note that this test pinned the defect was mistaken; the test that actually pinned it in prose was `test_brief_habit.py::test_week_bucketing_distinct_days_and_window`, whose comment read "``mornings_phone`` is 0 throughout … unattributed, not 'never read on the phone'" while asserting exact dicts that could not say so); (3) a JSON `null` would **not** be caught by HabitStrip's existing `!== undefined` guards — `null !== undefined` is `true` — so the Optional shape needed the same frontend edits **plus** a live risk of rendering `nullp`. It was the more dangerous shape, not the cheaper one. The gate is per-week and both directions are pinned, because on the live store both cases coexist: 07-13 and 07-20 are all-NULL (**false** zeros, `5m / 0p` today) while 08-03 is attributed with genuinely no phone reads (a **true** zero that must keep rendering — blanket-hiding zeros would suppress exactly the fade signal v14 exists to catch). An empty week is unattributed too: nothing measured it either. Frontend gates both the current-week "(N on phone)" clause and the history "/ Np"; the `PHONE_TITLE` tooltip that used to *explain* the false zero now documents that it is withheld. **RED shown before the fix and re-proved by replaying the final tests against base `af7f230`:** 5 fail there — 2 new backend tests (`KeyError: 'attributed'`), 2 new frontend tests (base renders `5 of 5 mornings ✓ (0 on phone)` and `5m / 0p / 1n`), plus the **updated** `test_week_bucketing_distinct_days_and_window`, which is an extended exact-dict pin rather than a third RED test and is labeled as such. Mutation check in isolation: forcing `attributed` always-true is killed by 2 tests. Backend 928 → 929, frontend 266 → 268 (both base figures measured by replaying `origin/main` at `af7f230`, not recalled), `ruff check` + `tsc` clean. **v1 criterion untouched** — `mornings` still counts every source, and the weeks certifying the ~08-19 verdict (08-03, 08-10) were already attributed, so the verdict was never at risk; only the July readout was._
...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #192, RED→green) — **nine review rounds, sixteen should-fix findings, fourteen of them introduced by the fix's own drafts.** Read the design off this note, not off the acceptance. A negated bound sets the **opposite** edge via `_NEG_END_TRIG`/`_NEG_START_TRIG` (Python's `re` requires lookbehinds fixed-width, which is why the originals were two literals seeing only the word before `after`). Every scan blanks each match out of a working copy and **repeats until exhausted**, so no occurrence survives into a later opposite-direction scan; blanking preserves length, so the **leftmost match per edge wins**, as the two single searches did. **Two negator classes**, split on whether the same word can also be spent elsewhere in the note: `no`/`not` are `_EXCLUDE_RE` day-exclusion triggers **and** carry an availability reading ("I have no meetings after 3pm" = those hours are FREE), so they flip a bound only across an allow-list of scheduling nouns (`more|sessions|blocks|study|studying`; `work` excluded as ambiguous); `nothing`/`never`/`none` are neither, so they read any ≤2 words — minus any word that opens a bound of its own or starts with a digit, which would otherwise swallow a range's opener into a zero-width window. **An unread `nothing|never|none` drops the WHOLE note** — checked per occurrence (a note-level flag let one readable clause vouch for an unreadable one), scoped to its own clause, and counting a **named window** as a time. Dropping only the *window* was never the honest floor it claimed to be: `_parse_days`/`_parse_session` kept the result non-empty, which short-circuits the `claude -p` lane in `api/study.py`, so the standing window the note was narrowing got planned and persisted with no message — on a busy calendar, booking blocks inside the excluded band. One router change rides along: `_apply_overrides` repairs an inverted window by moving the edge the note did **not** name. **The lesson worth carrying, and the reason this took nine rounds:** a gap or guard *miss* never produced "no reading" — it produced the **backwards** reading, confidently. Four separate rounds tried to give the guard a lexical exception for day exclusions ("nothing on Fridays" says nothing about hours, so it shouldn't veto a window stated beside it) — a punctuation test, a separator list, a clause-content test, a trigger word-list — and **every one closed one shape while re-opening another**, because the distinction is semantic and each proxy collided with a case meaning the opposite. **That exception was ultimately reverted rather than fixed a fifth time**, on a stopping rule agreed with Kyle in advance: a day negation that also reaches a time is now unread like any other, and the note goes to the fallback lane. The cost is honest over-caution on "nothing on Fridays and 9am to 5pm"; the alternative was another confident inversion. Two cases the card didn't name fall out too: `"not until 3pm"` (was `{15, 15}`) and `"nothing before 2pm"` (a full inversion). **RED verified by replaying the final tests against base `3f35a80`: 24 of the 27 touched tests fail there** (26 new + 1 pre-existing test **tightened** here). **The other three pass at base and are labelled the non-regression pins they are**, not counted as RED — each guards a regression a draft introduced. **Ten mutations, each applied and reverted in isolation, against the final 72-test set:** withholding removed kills 13 · blanking 12 · scan order 10 · negation set 10 · the unreadable signal dropping only the window 7 · named window not counted as a time 5 · per-occurrence check made note-level 3 · clause scoping dropped 3 · gap guard 1 · `_apply_overrides` repair 1. The scan-order figure is **the two negated scans moved after the two plain ones**, named precisely because an earlier sweep of it read 11: that run's extra failure was a router test the mutation had pushed onto the fallback lane, where it reached the **real** `claude` CLI — the parse-file count is a stable 10. One mutation earned its keep twice: dropping the clause scoping killed **zero** on an earlier sweep, exposing a genuinely unpinned shape rather than a redundant guard. **A live-LLM leak was caught in the process** — one router test had started passing by calling the real `claude` CLI once its note began falling through to the fallback; it now wires the failing fake and pins the honest degrade. Open follow-ups: `#192-F1` (distributed "nothing before 9am **or after 5pm**" drops the second bound) · `#192-F2` (an end-only note collapses the standing window to one hour and persists it) · `#192-F3` (the day-exclusion exception, reverted here — see its card for what four attempts proved). Backend 929→955 · ruff green · frontend untouched (no contract change)._
...
### Review follow-ups — 2026-08-13 (PR #192 adversarial review · 3 open)

_The nice-to-have findings from the pre-merge loop on PR #192 (08-12 hunt bug #5), plus the one
sub-guard that loop **reverted rather than fixed a fifth time** (`#192-F3`). That loop ran **nine
rounds** and found **sixteen should-fix defects** — fourteen of them introduced by the fix's own
drafts — every one of which was fixed and re-verified inside the PR. Full text in the review mailbox:
`~/.claude/reviews/home-base/2026-08-13-fix-studycal-parse-negation.md`._

#### [Review] #192-F1: A distributed negation ("nothing before 9am **or after 5pm**") silently drops the second bound
- **Where:** `backend/app/studycal/parse.py` (`_WINDOW_SCANS` + the leftmost-per-edge rule) · severity nice-to-have · confidence high
- **Why:** In the common "nothing before X **or** after Y" shape, only the first trigger carries an adjacent negation. The second is matched by the plain `_START_TRIG` pass and then discarded by the leftmost-wins guard, so the user's stated end bound vanishes and the standing (possibly much later) end is kept: `"nothing before 9am or after 5pm"` → `{'day_start_hour': 9}`, and with a persisted 21:00 end the learner gets 20:00 blocks from a note that said "nothing after 5pm". The explicitly-repeated form works (`"not before 2pm and not after 5pm"` → `{14, 17}`), so this is purely about distributing one negation across a conjunction. **Improved but not solved by #192** — the merge-base produced an inverted 17/9 pair here, so the leftmost rule is a real gain; the note is still half-ignored.
- **Acceptance:** Let a negated-bound match keep scanning past `or`/`and` for a second trigger of the **opposite** direction, and apply it to that edge. Regression test first; pin the standing-end case (a persisted 21:00 must not survive a "nothing after 5pm" clause).
- **Size:** S
- **Added:** 2026-08-13

#### [Review] #192-F3: A day exclusion joined by "and" costs the window it sits beside — the exception four rounds failed to specify
- **Where:** `backend/app/studycal/parse.py` (`_unread_time_exclusion`) · severity nice-to-have · confidence high
- **Why:** "nothing on Fridays" excludes DAYS and says nothing about hours, so it should not veto a window stated beside it. **Scope, stated precisely (review F27 corrected an earlier under-statement):** the drop bites any `nothing`/`never`/`none` **whose own clause also reaches a time** — not only the `and`/`but`-joined form. Clause scoping rescues it only when the negator's clause holds no time at all, so `"weekday evenings, nothing on Sundays"` survives (→ 17–21) while `"evenings, never on Sundays and no earlier than 6pm"` and `"weekday mornings, nothing on Fridays and no later than 5pm"` — both comma-scoped — drop just like `"nothing on Fridays and 9am to 5pm"`. In each case the note is handed to the `claude -p` lane, which on the deployed server is unreachable, so the user gets "I couldn't read that one" for a note the parser understood at the merge-base. **Over-caution, never an inversion** — that is the trade, taken deliberately. **PR #192 tried four times to specify the exception and reverted it:** a punctuation test (review F17) → a separator list including `and`/`but` (F20, F22) → a clause-content test on trigger words (F23, F24). Each closed one shape and re-opened another, because the distinction — "does this negator govern the hours, or only the days?" — is semantic, and every lexical proxy collided with a sentence meaning the opposite ("nothing on Saturdays **and after 3pm**" joins a second *excluded* thing; "nothing on Fridays **and** 9am to 5pm" joins a new statement; the two are indistinguishable by punctuation, separator or trigger word). The mailbox has the full evidence for each attempt.
- **Acceptance:** Only worth reopening with a genuinely different mechanism — e.g. scoping the veto to the keys the unread clause could have set (so a day negator can only ever cost `days_of_week`), or reading a negated day phrase as a real `_EXCLUDE_RE` exclusion so it stops being unread at all. **Do not attempt a fifth lexical proxy**; four rounds of evidence say it collides. Regression test first, and pin both the "and"-joined *and* the hours-on-days shape in the same test so the collision is visible.
- **Size:** M
- **Added:** 2026-08-13

#### [Review] #192-F2: An end-only note collapses the standing window to one hour and persists it
- **Where:** `backend/app/api/study.py:78-79` (the `_apply_overrides` inverted-window repair, as changed by #192) · severity nice-to-have · confidence high
- **Why:** #192 made the repair move the edge the note did **not** name, which stops an end-only note being "repaired" back into the band it excluded. But the replacement start is always `day_end_hour - 1`, so a standing 17→21 window plus `"nothing after 3pm"` becomes **14→15** — a one-hour band regardless of how wide the user's window was — and `set_study_prefs` writes it, so the original start is gone from the panel on the next visit. Combined with `max_per_day`/`session_minutes` this can leave steps unscheduled from a note that only meant "stop earlier". #192's own router test asserts only `day_start_hour < day_end_hour`, so the collapse is unpinned in either direction.
- **Acceptance:** Repair to a wider honest start — `min(_DEFAULT_START, end - 1)`, or the persisted start when it is already below the note's end — instead of always `end - 1`. Regression test first, pinning the *width* and not just the ordering.
- **Size:** S
- **Added:** 2026-08-13
