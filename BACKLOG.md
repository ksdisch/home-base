...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #192, RED→green) — **and the review is half the story: four rounds, seven should-fix findings, five of them regressions the fix's own drafts introduced.** Read the design off this note, not off the acceptance. A negated bound sets the **opposite** edge via `_NEG_END_TRIG`/`_NEG_START_TRIG` (Python's `re` requires lookbehinds fixed-width, which is why the originals were two literals that could only see the word immediately before `after`). Every scan blanks each match out of a working copy and **repeats until exhausted**, so no occurrence survives into a later opposite-direction scan; blanking preserves length, so offsets stay comparable and the **leftmost match per edge wins**, as the two single searches did. **Two negator classes**, split on whether the same word can also be spent elsewhere in the note: `no`/`not` are `_EXCLUDE_RE` day-exclusion triggers **and** carry an availability reading ("I have no meetings after 3pm" = those hours are FREE), so they flip a bound only across an allow-list of scheduling nouns (`more|sessions|blocks|study|studying`; `work` deliberately excluded as ambiguous); `nothing`/`never`/`none` are neither, so they read any ≤2 words — minus any word that opens a bound of its own or starts with a digit, which would otherwise swallow a range's opener into a zero-width window. **The structural lesson, and the one worth carrying forward:** four rounds of retuning that gap each traded one inverted phrasing set for another, because a gap *miss* never produced "no reading" — it produced the **backwards** reading, confidently, and a non-empty parse short-circuits the `claude -p` lane that might have read it. So a `nothing|never|none` that no negated scan could read now **withholds the window entirely** rather than answering the opposite; the note reaches the fallback lane and the controls hold. That also settles the negated *range* ("nothing from 9am until 5pm" is a workday — applying the range would schedule every block inside the band it rules out). One router change rides along: `_apply_overrides` repairs an inverted window by moving the edge the note did **not** name, so an end-only note can't be repaired back into its excluded band (the structured-controls repair at `study.py:297` is untouched). `by` stays out of the negated-start set — "not by 5pm" isn't a start phrasing. Two cases the card didn't name fall out too: `"not until 3pm"` (was `{15, 15}`) and `"nothing before 2pm"` (a full inversion). **RED verified by replaying the final tests against base `3f35a80`: 14 of the 15 touched tests fail there** (13 new + 1 pre-existing test **tightened** here — `test_exclusion_trigger_words_do_not_misfire_on_time_phrases` pinned only that no day override leaked out of "not before 2pm", leaving the spurious `day_end_hour` unguarded). The 15th, `test_a_negation_spent_on_a_day_exclusion_does_not_also_flip_a_time_bound`, **passes at base** — it guards a regression a draft introduced, so it is a non-regression pin, not a RED. **Six mutations, each applied and reverted in isolation, measured against the final 60-test set:** blanking removed kills 10 · negation set narrowed kills 8 · scan order flipped kills 8 · unreadable-exclusion withholding removed kills 3 · bound-opener/digit gap guard dropped kills 3 · the `_apply_overrides` repair reverted kills 1. Open follow-ups: `#192-F1` (distributed "nothing before 9am **or after 5pm**" drops the second bound) · `#192-F2` (an end-only note collapses the standing window to one hour and persists it). Backend 929→943 · ruff green · frontend untouched (no contract change)._
