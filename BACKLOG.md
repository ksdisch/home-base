...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #192, RED→green): both halves, exactly as the acceptance names them. A negated bound now sets the **opposite** edge through two new triggers — `_NEG_END_TRIG` (`nothing/never/none/not/no` … `after`) and `_NEG_START_TRIG` (… `before|until|till|til`) — built from a real negation set plus a **≤2-word gap**, because Python's `re` requires lookbehinds fixed-width and the gap is precisely what "no sessions after 3pm" needs (the two literals this replaces could only see the word immediately before `after`). The four scans run against a working copy with **each accepted match blanked out** — `_parse_days`' own trick — so one phrase can never satisfy two triggers; blanking preserves string length, so offsets stay comparable and the **leftmost match per edge wins**, which is exactly what the two single searches did (an important non-change: "not before 2pm or after 5pm" still reads 2pm as the start rather than being overwritten by the later bare `after`). **The negated forms are scanned first**, because each one's own trigger word belongs to the other direction. `by` was deliberately **not** added to the negated-start set — "not by 5pm" isn't a start phrasing in English; END's bare `by` is protected structurally by the blanking instead. Two cases the card didn't name are fixed by the same pass: `"not until 3pm"` (was `{15, 15}`) and `"nothing before 2pm"` (was a full inversion to `day_end_hour: 14`). **RED verified by replaying the final tests against base `3f35a80`: 5 fail there** — 4 new (3 parse + 1 router), and the fifth, `test_exclusion_trigger_words_do_not_misfire_on_time_phrases`, is a **pre-existing test tightened here** (it pinned only that no day override leaked out of "not before 2pm" and left the spurious `day_end_hour` unguarded), so it is labelled an updated pin rather than counted as a fifth RED. **Four mutations, each applied and reverted in isolation:** narrowing `_NEG` back to `not|no` kills 4 · removing the ≤2-word gap kills 2 · removing the blanking kills all 5 · flipping the scan order kills all 5. Backend 929→933 · ruff green · frontend untouched (no contract change)._
