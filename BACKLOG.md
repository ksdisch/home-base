...
- _✅ fixed 2026-07-27 (PR #157, RED→green): a module-level `threading.Lock` now spans the whole read→grade→append critical section with the ledger re-read inside it, and `_read_ledger` collapses to one row per (day, slug, headline) so a duplicate already on disk stops double-counting. The two-thread test is deterministic, not hopeful — it fails 5/5 with the lock removed and passes 5/5 with it. Landed with the paired Harden re-grade fix; both touch `_read_ledger` semantics._
