...
- **Size:** S
- **Added:** 2026-08-12
- _✅ fixed 2026-08-13 (PR #192, RED→green) — **and the review is half the story: eight rounds, fourteen should-fix findings, twelve of them introduced by the fix's own drafts** (only the original inversion and its API-level consequence were in the code before this branch). Read the design off this note, not off the acceptance. A negated bound sets the **opposite** edge via `_NEG_END_TRIG`/`_NEG_START_TRIG` (Python's `re` requires lookbehinds fixed-width, which is why the originals were two literals that could only see the word immediately before `after`). Every scan blanks each match out of a working copy and **repeats until exhausted**, so no occurrence survives into a later opposite-direction scan; blanking preserves length, so offsets stay comparable and the **leftmost match per edge wins**, as the two single searches did. **Two negator classes**, split on whether the same word can also be spent elsewhere in the note: `no`/`not` are `_EXCLUDE_RE` day-exclusion triggers **and** carry an availability reading ("I have no meetings after 3pm" = those hours are FREE), so they flip a bound only across an allow-list of scheduling nouns (`more|sessions|blocks|study|studying`; `work` deliberately excluded as ambiguous); `nothing`/`never`/`none` are neither, so they read any ≤2 words — minus any word that opens a bound of its own or starts with a digit, which would otherwise swallow a range's opener into a zero-width window. **The structural lesson, and the one worth carrying forward:** four rounds of retuning that gap each traded one inverted phrasing set for another, because a gap *miss* never produced "no reading" — it produced the **backwards** reading, confidently, and a non-empty parse short-circuits the `claude -p` lane that might have read it. So an **unread** `nothing|never|none` now withholds the window entirely. Getting that guard right took three more rounds of its own, each miss for the same reason: it was specified by enumeration rather than by the question it means to ask. It is checked **per occurrence** (a note-level flag let one readable clause vouch for an unreadable one — "nothing from work after 3pm, not before 2pm" read the 2pm and answered the 3pm backwards); a negator whose next tokens are a **day phrase is skipped structurally**, with no punctuation involved (a bare presence test let "weekday evenings, nothing on Sundays" veto the evening band, and a comma-only clause rule then answered "nothing on Fridays **and** 9am to 5pm" differently from the same note with a comma); and "governs a time" counts a **named window** as a time, since "nothing in the afternoon" carries no digits and reading it as a *selection* schedules exactly the band it rules out. A negator that excludes days is skipped **only when the day list is where it ends** — "nothing on sat or sun after 3pm" excludes hours *on* days, and skipping it hands that `after` to the forward scan. "Where it ends" is decided by what the rest of the clause **contains**, not by which separator follows: `and`/`but` joins a second *excluded* thing ("nothing on Saturdays and after 3pm") exactly as readily as a new statement, and two rounds of evidence say no separator list separates those. A bare flippable trigger (`after|before|until|till|til|by`) anywhere in the clause blocks the skip, because an orphaned one inherits the negation; a self-contained bound ("no later than 5pm") or a plain range carries its own polarity and does not. A named window blocks it only while still **attached** to the day list — after a conjunction it belongs to the new statement. And the deepest correction, which is what finally ended the cycle: **an unread exclusion drops the WHOLE note, not just the window.** Withholding only the window was never the honest floor it claimed to be — `_parse_days`/`_parse_session` kept the result non-empty, which short-circuits the `claude -p` lane in `api/study.py`, so the standing window the note was trying to narrow got planned and persisted with no message at all. On a busy calendar that booked blocks inside the excluded band. That rule also settles the negated *range*: "nothing from 9am until 5pm" is a workday, and applying it would schedule every block inside the band it rules out. One router change rides along: `_apply_overrides` repairs an inverted window by moving the edge the note did **not** name, so an end-only note can't be repaired back into its excluded band (the structured-controls repair at `study.py:297` is untouched). `by` stays out of the negated-start set — "not by 5pm" isn't a start phrasing. Two cases the card didn't name fall out too: `"not until 3pm"` (was `{15, 15}`) and `"nothing before 2pm"` (a full inversion). **RED verified by replaying the final tests against base `3f35a80`: 24 of the 29 touched tests fail there** (28 new + 1 pre-existing test **tightened** here — `test_exclusion_trigger_words_do_not_misfire_on_time_phrases` pinned only that no day override leaked out of "not before 2pm", leaving the spurious `day_end_hour` unguarded). **The other five pass at base and are labelled the non-regression pins they are**, not counted as RED: every one guards a regression a draft introduced (the day-exclusion hijack · the day-clause veto in its comma form · the same veto in its conjunction form), so a passing pin there is the correct outcome, not a missing RED. **Fourteen mutations, each applied and reverted in isolation, measured against the final 74-test set:** withholding removed entirely kills 13 · blanking removed 12 · negation set narrowed 10 · scan order flipped 10 · the unreadable signal dropping only the window rather than the note 7 · named window no longer counted as a time 6 · the day skip made unconditional again 4 · its flippable-trigger half dropped 4 · the skip removed entirely 3 · per-occurrence check made note-level 2 · its attached-named-window half dropped 2 · clause scoping dropped 1 · bound-opener/digit gap guard dropped 1 · `_apply_overrides` repair reverted 1. **One of them earned its keep twice:** dropping the clause scoping killed **zero** tests on the first sweep, which exposed a genuinely unpinned shape (`"nothing fancy, 9am to 5pm"` — a negator governing no time at all) rather than a redundant guard; a pin was added and the mutation now kills 1. Open follow-ups: `#192-F1` (distributed "nothing before 9am **or after 5pm**" drops the second bound) · `#192-F2` (an end-only note collapses the standing window to one hour and persists it). Backend 929→957 · ruff green · frontend untouched (no contract change)._
...
### Review follow-ups — 2026-08-13 (PR #192 adversarial review · 2 open)

_The two nice-to-have findings from the pre-merge loop on PR #192 (08-12 hunt bug #5). That loop ran
the full three rounds and found **four should-fix defects** — three of them regressions the fix's own
drafts introduced — all of which were fixed and re-verified inside the PR, so only these two are
carried. Full text in the review mailbox:
